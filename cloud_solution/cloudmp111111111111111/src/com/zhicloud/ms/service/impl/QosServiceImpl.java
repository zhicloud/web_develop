...
public class QosServiceImpl implements IQosService{
    ...
    @Override public MethodResult addQos(Map<String, Object> parameter) {
        ...
        try {
            ...
            JSONObject hostInfo = (JSONObject) data.get("host");
            
            //码率，帖率，系统类型，操作系统
            codeRate = Integer.valueOf(hostInfo.get("ibt").toString());
            frameRate = Integer.valueOf(hostInfo.get("fram").toString());
            operating_type = Integer.valueOf(hostInfo.get("operating_type").toString());
            operating_system = hostInfo.get("operating_system").toString();
        ...
        try {
            JSONObject result = channel.hostModify(uuid, "", 0, BigInteger.ZERO, new Integer[0],
                new Integer[0], "", "", "", inboundBandwidth, outboundBandwidth, maxIops, priority,codeRate,frameRate,operating_type,operating_system);
    ...
    @Override public MethodResult removeQos(List<String> ids) {
        ...
        while (it.hasNext()) {
            ...
            try {
                ...
                result = channel.hostModify(uuid, "", 0, BigInteger.ZERO, new Integer[0], new Integer[0], "", "", "", BigInteger.ZERO, BigInteger.ZERO, 0, 0,codeRate,frameRate,operating_type,operating_system);
