...
public class ResourcePoolController {
    ...
    public String hostPoolDetail(@PathVariable("uuid") String uuid,@PathVariable("name") String name,Model model,HttpServletRequest request){
        ...
        try {
                ...
                HttpGatewayChannelExt channel = HttpGatewayManager.getChannel(1);
                if(channel!=null){
//                    JSONObject result = channel.hostQuery(uuid,name,2);
...
//
   ...
   * @author 张翔
    ...
    * @throws
     ...
     * @throws
     */
    @RequestMapping(value="/{id}/hostMigration",method=RequestMethod.GET)
    public String hostMigration(@PathVariable("id") String id,HttpServletRequest request,Model model){
    	if( ! new TransFormPrivilegeUtil().isHasPrivilege(request, TransFormPrivilegeConstant.desktop_resource_host_query)){
            return "not_have_access";
        }
    ...
    @ResponseBody
    public MethodResult delMigration(@PathVariable("id") String id,Model model,HttpServletRequest request){
    	
    	MethodResult mr = new MethodResult(MethodResult.FAIL,"删除失败");
        
        if(!"".equals(id) && null !=id){
        	if(hostMigrateionService.delById(id)>0){
    ...
    @ResponseBody
    public MethodResult toMigration(HttpServletRequest request,Model model){
    	...
    	String hostName = request.getParameter("displayName");
    	String type = request.getParameter("type");
    	String localhostNC = request.getParameter("innerIp");
    	Integer hostType = 0;
    	
    	if(!"".equals(type) && null != type)
    		hostType = Integer.valueOf(type);
...
//    	condition.put("toNC", localhostNC);
    	...
    	if(ids!=null && !"".equals(ids)){
    		String[] realNameId = ids.split(",");
    		String toNC = "";
    		for(String uuid : realNameId){
    			CloudHostVO host = cloudHostService.queryCloudHostById(uuid);
    			HttpGatewayChannelExt channel = HttpGatewayManager.getChannel(1);
                if(channel!=null){
                	JSONObject result;
					try {
						...
						//sessionId = this.migration(uuid, disk, mode, host.getSysImageId());
						if (sessionId == null) {
			            ...
			            AppInconstant.migrationProgress.put(uuid + "_reset_progress", temporaryData);
    			...
    			mr = cloudHostService.operatorMigration(uuid, condition);
                }
	...
	public String migration(String uuid, int disk, int mode, String image) throws MalformedURLException, IOException {
		HttpGatewayAsyncChannel channel = HttpGatewayManager.getAsyncChannel(1);
		try {

			JSONObject result = channel.hostFlushDiskMigration(uuid, disk, mode, image);
			if (HttpGatewayResponseHelper.isSuccess(result) == true) {
				...
				if (migration == null) {
					...
					migration.setSessionId(null);
					migration.setResetStatus(1);
					pool.put(migration);
				}
				return channel.getSessionId();
			} else {
				System.err.println("fail to send flush disk migration request.");
				channel.release();
			}
		} catch (Exception e) {
			System.err.println("fail to send flush disk migration request.");
