...
//      JSONObject result = HttpGatewayHelper.getInstance().hostAttachDisk("7cf61147-1b1d-40b4-a76a-625db196eb80", new BigInteger(1024*1024*1024+""), 0, "", 0);
    ...
    public JSONObject hostFlushDiskMigration(String uuid, int disk, int mode, String image, String callback) throws MalformedURLException, IOException {
    	String url = this.baseUrl + "host?";
    	url += "command=flush_disk_migrate&";
    	url += "session_id=" + this.sessionId + "&";
...
public JSONObject hostMigrate(String uuid, String target, int type, String callback) throws MalformedURLException, IOException {
    ...
    public JSONObject imageServiceQuery(String callback) throws MalformedURLException, IOException {
        String url = this.baseUrl + "image_service?";
        url += "command=query&";
        url += "session_id=" + this.sessionId + "&";

        if (logger.isDebugEnabled()) {
            logger.info("query_image_service() -> url=[" + url + "]");
        }
