...
//				task, server, host, cpuCount, cpuUsage, memory, memoryUsage, diskVolume, diskUsage, diskIO, networkIO, speed, timestamp));
	...
	//主机迁移begin
	@HttpGatewayMessageHandler(messageType = "flush_disk_migrate_ack")
    public void flushDiskMigrationAck(HttpGatewayAsyncChannel channel, JSONObject messageData) {
        ...
        AppInconstant.migrationProgress.put(uuid+"_reset", "reset_true");
        ...
        logger.info(String.format("start to flush disk migration. uuid[%s], disk[%d]", uuid, disk));
    }

    @HttpGatewayMessageHandler(messageType = "flush_disk_migrate_progress")
    public void flushDiskMigrationProgress(HttpGatewayAsyncChannel channel, JSONObject messageData) {
        logger.debug("recieve flush disk migration progress response.");
        ...
        migration.updateTime();
        channel.release();
        logger.error(String.format("flush disk migration at progress[%d]. uuid[%s], disk[%d]", level, uuid, disk));
    }
	
	@HttpGatewayMessageHandler(messageType = "flush_migrate_image")
    public void flushDiskMigration(HttpGatewayAsyncChannel channel, JSONObject messageData) {
        logger.debug("recieve flush disk migration response.");
        ...
        if (HttpGatewayResponseHelper.isSuccess(messageData) == true) {
            ...
            logger.info(String.format("flush disk migration success. uuid[%s], disk[%d], success", uuid, disk));
        } else {
            migration.setSuccess(false);
            logger.error(String.format("[%s]flush disk migration fail. uuid[%s]", sessionId, uuid));
        }
        AppInconstant.migrationProgress.put(uuid+"_reset", "reset_false");
        channel.release();
