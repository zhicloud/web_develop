...
jQuery(document).ready(function(){
	...
	jQuery('.vernav2 ul ul li a').click(function(){
		...
		//remove current menu
		jQuery('.vernav2 ul ul li').each(function(){
			jQuery(this).removeClass('current');
		});
	        ...
	        success: function(result)
	        {
	        	if(result.status == "fail"){
	        		jAlert('登录或以超时，请重新登录','提示', function(r) {
		        		...
		        		// 主机类型管理
		        		if( target.id=="host_type_manage" )
		        		{
		        			jQuery("#content_frame").attr("src", "../chcm/all");
		        		}
		        		//镜像管理
		        		if( target.id=="image_manage_menu" )
		        		{
		        			jQuery("#content_frame").attr("src", path+"/image/imagelist");
		        		}
		                //主机管理
		        		if( target.id=="cloud_host_ware" )
		        		{
		        			window.location.href = path+"/warehouse/all";
//		        			jQuery("#content_frame").attr("src", "../warehouse/all");
		        		}
		        		//分组管理
		        		if( target.id=="sys_group_manage" )
		        		{
		        			jQuery("#content_frame").attr("src", "../group");
		        		}  
		        		//用户列表
		        		if( target.id=="terminal_user_manage" )
		        		{
		        			jQuery("#content_frame").attr("src", "../user/list");
		        		}
		        		//版本控制
		        		if( target.id=="version_record_manage" )
		        		{
		        			jQuery("#content_frame").attr("src", "../version/list");
		        		}
		        	} 
	        }
	     });
...
//显示隐藏的元素，disabled一个元素
function showAndDisabled(showId,disabledId){
     jQuery("#"+showId).show();
     if(disabledId!=""){
         // 禁用该元素
         jQuery("#"+disabledId).attr("disabled","disabled");
         // 将该元素border颜色恢复成之前的颜色
         jQuery("#"+disabledId).css("border-color","#ccc");
     }
     
 }

//隐元素，对一个元素解除禁用
function hideAndAbled(hideId,abledId){
     jQuery("#"+hideId).find("input select").each(function () {
        jQuery(this).css("border-color","#ccc"); 
     });
     jQuery("#"+hideId).hide();
     if(abledId!=""){
         // 解除禁用该元素
         jQuery("#"+abledId).removeAttr("disabled");
     }
     
 }
//对一个元素disabled,对另一个元素abled
function abledAndDiabled(abledId,disabledId){
    if(abledId!=""){
        jQuery("#"+abledId).removeAttr("disabled"); 
    }
    if(disabledId!=""){
