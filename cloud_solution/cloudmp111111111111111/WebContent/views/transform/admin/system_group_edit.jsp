...
var tempbillid = "${systemGroup.billid}";
...
function saveForm(){
	...
	if(form.parsley('isValid')){
		...
		var name = $("#name").val();
		var param = "name="+name;
		var url = "<%=request.getContextPath()%>/transform/groupadmin/savegroup"
		jQuery.ajax({
	  	 	type: "POST",
	  	 	async:false,
	   		url: url,
	     	...
	     	if(obj.status=="success"){
	     		window.location.href = "<%=request.getContextPath()%>/transform/groupadmin/index";
	     	}else{
...
<!-- Page content -->
        <div id="content" class="col-md-12">
          ...
          <div class="pageheader">
            ...
            <c:if test="${empty systemGroup}">新增角色组</c:if>
            <c:if test="${!empty systemGroup}">修改角色组</c:if>
            </h2>
              ...
              <div class="col-md-12">
                ...
                <section class="tile color transparent-black">
                  ...
                  <div class="tile-header">
                    <h3><a href="<%=request.getContextPath() %>/transform/groupadmin/index"    style="color:#FAFAFA;cursor:pointer;padding-right:10px;"> <i class="fa fa-reply"></i></a>
                        <c:if test="${empty systemGroup}">输入角色信息</c:if>
                        <c:if test="${!empty systemGroup}">修改角色组信息</c:if></h3>
                    <div class="controls">
                    ...
                    <form class="form-horizontal" role="form" parsley-validate id="basicvalidations">
                     <c:if test="${empty systemGroup}">
                      ...
                      <div class="form-group">
                        <label for="name" class="col-sm-2 control-label">角色组名称*</label>
                        <div class="col-sm-4">
                     ...
                     <c:if test="${!empty systemGroup}">
                      ...
                      <div class="form-group">
                        <label for="name" class="col-sm-2 control-label">角色组名称*</label>
                        <div class="col-sm-4">
                          <input value="${systemGroup.name }" type="text" class="form-control" id="name" parsley-required="true"  parsley-maxlength="100">
                        </div>
                      </div>
                      </c:if>                      
                      <!-- modify -->
