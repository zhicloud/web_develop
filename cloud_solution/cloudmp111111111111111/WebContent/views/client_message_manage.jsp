      ...
      <div class="row">
        ...
        <div id="content" class="col-md-12">
          ...
          <div class="pageheader">
            ...
            <h2><i class="fa fa-list-alt"></i> 反馈信息管理</h2>
          ...
          <div class="col-md-12">
                  ...
                  <!-- /tile header -->
                    ...
                    <button type="button" class="btn btn-green file-excel-o" onclick="exportData('/export/clientmessagedata')">
                    ...
                    <div class="row">
                      ...
                      <div class="col-sm-3 col-xs-6" style="z-index: 100;">
                        ...
                        <span class="input-group-btn">
                          <select id="message_type" class="chosen-select form-control">
                            <option value="">类型(全部)</option>
                            <option value="1">故障反馈</option>
                            <option value="2">意见建议</option>
                            <option value="3">其它</option>
                          </select>
                          <select id="message_status" class="chosen-select form-control">
                      ...
                      <table  class="table table-datatable table-custom" id="basicDataTable">
                        ...
                        <c:forEach items="${cmList}" var="cm">
                          ...
                          <tr class="odd gradeX">
						          <td>
									<div class="checkbox check-transparent">
									  <input type="checkbox" name="idcheck" value="${cm.id}" id="${cm.id}">
									  <label for="${cm.id}"></label>
									</div>
                                 </td>
                                  <td>${cm.userName}</td>
								  <td class="cut">${cm.content }</td>
								  <td>${cm.getTypeFormat() }</td> 
								  <td>${cm.getStatusFormat() }</td> 
								  <td>${cm.getCreateTimeFormat() }</td> 
                          </tr>
                        </c:forEach>
                        </tbody>
                      </table>
...
// 	      $("#con").click();
	  ...
	  jQuery("#save_oper").click(function(){
		  	...
		  	if(option=="del"){
		        ...
		        if(ids.length==0){
		        	$("#tipscontent").html("请选择要删除的信息");
					$("#dia").click();
		       	 	return;
		        } 
				curIds = ids;
				$("#confirmcontent").html("确定要删除所选反馈信息吗？");
		    	$("#confirm_btn").attr("onclick","deleteMultMessage();");
		    	$("#con").click();
		  	}else if(option=="read"){
		  		var ids = [];
				 var datatable = $("#basicDataTable").find("tbody tr input[type=checkbox]:checked");
		        ...
		        if(ids.length==0){
		        	$("#tipscontent").html("请选择要标记已读的信息");
					$("#dia").click();
		       	 	return;
		        } 
				curIds = ids;
				$("#confirmcontent").html("确定要标记所选信息为已读吗？");
		    	$("#confirm_btn").attr("onclick","readMultMessage();");
		    	$("#con").click();
		  	}
	     });
	  ...
	  $("#search_btn").click(function(){
		  ...
		  var status = $("#message_status").val();
		  window.location.href = path + "/clientmessage/all?type="+type+"&status="+status;
	  });
    ...
    function deleteMultMessage(){
    	jQuery.ajax({
	        url: '<%=request.getContextPath()%>/clientmessage/deleteMessageByIds',
	        type: 'post', 
	        dataType: 'json',
	        data:{ids:curIds},
	        timeout: 10000,
    ...
    function readMultMessage(){
    	jQuery.ajax({
	        url: '<%=request.getContextPath()%>/clientmessage/markReadByIds',
	        type: 'post', 
	        dataType: 'json',
	        data:{ids:curIds},
	        timeout: 10000,
