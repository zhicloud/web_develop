    ...
    var h = function(a) {
        this.messages = {
            ...
            type: {
                ...
                number: "请输入数字",
                integer: "请输入整数",
                nochinese: "请输入字母、数字和特殊字符",
                digits: "请输入数字",  
                dateIso: "This value should be a valid date (YYYY-MM-DD).",
                alphanum: "This value should be alphanumeric.",
                phone: "请输入正确的手机号",
                sendtime:"请输入【24:00:00】格式的定时发送时间</b>",
                sharedurl:"请输入【172.18.10.1:/mnt/develop/】格式的路径"
            },
            ...
            equalto: "确认密码和密码不一致",
            checkdesktoptypename:"该配置名称已经存在",
            checkdesktopimagename:"该镜像名称已经存在", 
            checkversionname:"版本名称不符合规则，请按以下规则填写", 
            checkdesktopwarehousename:"该仓库名称已经存在",
            checkusername:"该用户名已经存在",
            checkalias:"该别名已经存在",
            checkdesktopgroupname:"该分组名称已经存在",
            checkdboxserial:"该编号已存在",
            checkdesktoprecomputersourcepoolname:"该资源池名称已经存在",
            checkemailconfigname:"该配置名已存在",
            checkemailtemplatename:"该模板已存在",
            checkemailtemplatecode:"该代码已存在",
            checksmsconfigname:"该配置名已存在",
            checksmstemplatename:"该模板已存在",
            checksmstemplatecode:"该代码已存在",
            checkhostdisplayname:"该显示名已存在",
            checkqosname:"该规则已存在",
            checkblacklistname:"该规则已存在",
            checkwhitelistname:"该规则已存在",
            checkip:"IP格式不正确"
...
//                        	c = /^((\+\d{1,3}(-| )?\(?\d\)?(-| )?\d{1,5})|(\(?\d{2,6}\)?))(-| )?(\d{3,4})(-| )?(\d{4})(( x| ext)\d{1,5}){0,1}$/;
                        ...
                        case "sendtime":
                        	...
                        	break;
                        case "sharedurl":
                        	c = /^((?:(?:25[0-5]|2[0-4]\d|((1\d{2})|([1-9]?\d)))\.){3}(?:25[0-5]|2[0-4]\d|((1\d{2})|([1-9]?\d))):((\/(.*))+)\/)$/;
                        	break;	
                        default:
                            return ! 1
                        }
            ...
            ip: function() {
            	...
            	return {
            		validate: function(a) {
            			return /^((25[0-5]|2[0-4]\d|[01]?\d\d?)($|(?!\.$)\.)){4}$/.test(a)
            		},
            		priority: 32
            	}
            },
            port: function() {
                ...
                return {
                    validate: function(a) {
                        return /^([1-9]\d{0,3}|[1-5]\d{4}|6[0-5]{2}[0-3][0-5])(-([1-9]\d{0,3}|[1-5]\d{4}|6[0-5]{2}[0-3][0-5]))?$/.test(a)
                    },
                    priority: 32
                }
            },
            	...
            	return {
            		validate: function(a) {
            			...
            			if(array.length<4){
            				...
            				flag = /^[A-Z]+$/.test(array[0]);
            				if(flag == false){
            					return flag;
            				}
            				flag = /^[A-Z]+$/.test(array[2]);
            				if(flag == false){
            					return flag;
            				}
            				flag = /^[A-Z0-9]+$/.test(array[3]);
            				if(flag == false){
            					return flag;
            				}
            				flag = /^[0-9]{0}([0-9]|[.])+$/.test(array[1]);
            				return flag;
            			}
            ...
            checkdesktoptypename: function() {
            	return {
                    validate: function(a, b, c) {
                    	var str = c.element[0].outerHTML;
                    	if(str.indexOf('oldname="'+a+'"')!=-1){
                    		return true;
                    	}
                    	var flag;
                        jQuery.ajax({
            ...
            checkdesktopimagename: function() {
            	return {
            		validate: function(a, b, c) {
            			var str = c.element[0].outerHTML;
            			if(str.indexOf('oldname="'+a+'"')!=-1){
            ...
            checkdesktopgroupname: function() {
            	return {
            		validate: function(a, b, c) {
            			var str = c.element[0].outerHTML;
            			if(str.indexOf('oldname="'+a+'"')!=-1){
            ...
            checkdesktoprecomputersourcepoolname: function() {
                return {
                    validate: function(a, b, c) {
                        var str = c.element[0].outerHTML;
                        if(str.indexOf('oldname="'+a+'"')!=-1){
            ...
            checkdesktopwarehousename: function() {
            	return {
                    validate: function(a, b, c) {
                    	var str = c.element[0].outerHTML;
                    	if(str.indexOf('oldname="'+a+'"')!=-1){
            ...
            checkhostdisplayname: function() {
            	return {
                    validate: function(a, b, c) {
                    	...
                    	var flag;
                        jQuery.ajax({
                        	url:path + "/warehouse/cloudhost/checkHostDisplayName",
                            type: 'post',
                            data: {displayName:a},
                            dataType: 'json', 
                            async: false,
            ...
            checkemailconfigname: function() {
                return {
                    validate: function (a, b, c) {
                        ...
                        var flag;
                        jQuery.ajax({
                            url: path + '/message/email/config/checkname',
                            type: 'post',
                            data: 'name=' + a,
                            dataType: 'json',
                            async: false,
            ...
            checkemailtemplatename: function() {
                return {
                validate: function (a, b, c) {
                    var str = c.element[0].outerHTML;
                    if (str.indexOf('oldname="' + a + '"') != -1) {
                    ...
                    var flag;
                    jQuery.ajax({
                        url: path + '/message/email/template/checkname',
                        type: 'post',
                        data: 'name=' + a,
                        dataType: 'json',
                        async: false,
            ...
            checkemailtemplatecode: function() {
                return {
                    validate: function (a, b, c) {
                        ...
                        var flag;
                        jQuery.ajax({
                            url: path + '/message/email/template/checkcode',
                            type: 'post',
                            data: 'name=' + a,
                            dataType: 'json',
                            async: false,
            ...
            checksmsconfigname: function() {
                return {
                    validate: function (a, b, c) {
                        ...
                        var flag;
                        jQuery.ajax({
                            url: path + '/message/sms/config/checkname',
                            type: 'post',
                            data: 'name=' + a,
                            dataType: 'json',
                            async: false,
            ...
            checksmstemplatename: function() {
                return {
                    validate: function (a, b, c) {
                        ...
                        var flag;
                        jQuery.ajax({
                            url: path + '/message/sms/template/checkname',
                            type: 'post',
                            data: 'name=' + a,
                            dataType: 'json',
                            async: false,
            ...
            checksmstemplatecode: function() {
                return {
                    validate: function (a, b, c) {
                        ...
                        var flag;
                        jQuery.ajax({
                            url: path + '/message/sms/template/checkcode',
                            type: 'post',
                            data: 'name=' + a,
                            dataType: 'json',
                            async: false,
            ...
            checkblacklistname: function() {
                return {
                    validate: function (a, b, c) {
                        ...
                        var flag;
                        jQuery.ajax({
                            url: path + '/networkrule/blacklist/checkname',
                            type: 'post',
                            data: 'name=' + a,
                            dataType: 'json',
                            async: false,
            ...
            checkwhitelistname: function() {
                return {
                    validate: function (a, b, c) {
                        ...
                        var flag;
                        jQuery.ajax({
                            url: path + '/networkrule/whitelist/checkname',
                            type: 'post',
                            data: 'name=' + a,
                            dataType: 'json',
                            async: false,
            ...
            checkip: function() {
            	return {
            		validate: function (a, b, c) {
            			...
            			var flag;
            			jQuery.ajax({
            				url: path + '/networkrule/blacklist/checkip',
            				type: 'post',
            				data: 'ip=' + a,
            				dataType: 'json',
            				async: false,
            ...
            checkqosname: function() {
                return {
                    validate: function (a, b, c) {
                        ...
                        var flag;
                        jQuery.ajax({
                            url: path + '/networkrule/qos/check?type='+type,
                            type: 'get',
                            data: 'name=' + a,
                            dataType: 'json',
                            async: false,
