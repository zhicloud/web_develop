      ...
      <div class="row">
        ...
        <div id="content" class="col-md-12">
          ...
          <div class="pageheader">
            ...
            <h2><i class="fa fa-desktop"></i> 云服务器管理</h2>
          ...
          <div class="col-md-12">
                  ...
                  <!-- /tile header -->
                    ...
                    <button type="button" class="btn btn-green file-excel-o" onclick="exportData('/export/cloudserverdata')">
                  ...
                  <div class="tile-body no-vpadding"> 
                    ...
                    <div class="table-responsive">
                      ...
                      <table  class="table table-datatable table-custom" id="basicDataTable">
                        ...
                        <tbody>
                        <c:forEach items="${cloudServerList}" var="serverList">
                          ...
                          <tr class="odd gradeX">
                                  ...
                                  <td class="cut">${serverList.tenantName}</td>
								  ...
								  <c:choose>
								  	...
								  	<c:when test="${serverList.getSummarizedStatusText() eq '正在重装' }">
								  	<td><span class="label label-danger">正在重装</span></td>
                                  ...
                                  <ul class="dropdown-menu" role="menu">
		                            ...
		                            <c:if test="${serverList.status==11}">
			                              <li><a href="javascript:void(0);" onclick="flushManageBtn('${serverList.id }');">重装系统</a></li>
		                            </c:if>
		                            <c:if test="${serverList.status==2}">
		                                 ...
		                                 <c:if test="${serverList.runningStatus==1}">
			                              ...
			                              <li><a href="javascript:void(0);" onclick="snapshotBtn('${serverList.realHostId }');">快照管理</a></li>
			                              <li><a href="javascript:void(0);" onclick="backupHostBtn('${serverList.id }');">备份与恢复</a></li>
			                              <li><a href="javascript:void(0);" onclick="flushManageBtn('${serverList.id }');">重装系统</a></li>
...
<%-- 			                              <li><a href="javascript:void(0);" onclick="tomigrate('${serverList.realHostId }');" title="迁移">迁移</a></li> --%>
		                                ...
		                                <c:if test="${serverList.runningStatus==2}">
		                                 ...
		                                 <c:if test="${serverList.realHostId!=null}">
		                                 	<li><a href="javascript:void(0);" onclick="serverDiagramBtn('${serverList.realHostId }');">资源监控</a></li>
		                                 </c:if>
		                                  <li class="divider"></li>
		                              <li><a href="javascript:void(0);" onclick="deleteHostBtn('${serverList.id }');" >删除</a></li>                          	
		                              <li><a href="javascript:void(0);" onclick="createImageBtn('${serverList.id }');">制作镜像</a></li>                          	
		                             </c:if>
                    ...
                    <div class="modal fade" id="modalhostallocate" tabindex="-1" role="dialog" aria-labelledby="modalConfirmLabel" aria-hidden="true">
                      <div class="modal-dialog">
                        <div class="modal-content">
                          ...
                          <div class="modal-body">
                            <form class="form-horizontal" role="form" parsley-validate id="basicvalidations_allocate" action="<%=request.getContextPath() %>/cloudserver/settenant" method="post"   >
    ...
    $(function(){
	  ...
	  $("#add_server").click(function(){
		  window.location.href = path+"/cloudserver/add";
 	  });
...
// 		        })
  ...
  //强制重启主机
    ...
    function backupHostBtn(id){
    	window.location.href = path+"/cloudserver/"+id+"/backupManage";
    }
    //强制关机
    ...
    function updateHostBtn(id,run_status){
    	...
    	if(run_status=="2"){
    		...
    		window.location.href = path+"/cloudserver/"+currentId+"/"+runStatus+"/update";
    	}
    }
    //磁盘管理
    function diskManageBtn(id){
    	window.location.href = path+"/cloudserver/"+id+"/diskManage";
    }
    function flushManageBtn(id){
    	window.location.href = path+"/cloudserver/"+id+"/flushdiskimage";
    }
    //资源监控
    function serverDiagramBtn(id){
    	window.location.href = path+"/cloudserver/"+id+"/diagram";
    }
  //迁移
    function tomigrate(id){
    	window.location.href = path+"/cloudserver/"+id+"/migrate";
    }
    //制作镜像
    ...
    //磁盘快照
    function snapshotBtn(id){
    	window.location.href = path+"/snapshot/"+id+"/query";
    }  
  ...
  //分配给单个用户
    function assignOneUserBtn(id){
      ...
      function deleteHost(){
    	jQuery.get(path + "/cloudserver/"+currentId+"/delete",function(data){
			if(data.status == "success"){   
    ...
    function startHost(){
    	jQuery.get(path + "/cloudserver/"+currentId+"/start",function(data){
			if(data.status == "success"){   
	    		location.href = path + "/cloudserver/all";
			}else{  
				$("#tipscontent").html(data.message);
    ...
    function restartHost(){
    	jQuery.get(path + "/cloudserver/"+currentId+"/restart",function(data){
			if(data.status == "success"){   
	    		location.href = path + "/cloudserver/all";
			}else{  
				$("#tipscontent").html(data.message);
    ...
    function resetHost(){
    	jQuery.get(path + "/cloudserver/"+currentId+"/reset",function(data){
			if(data.status == "success"){   
	    		location.href = path + "/cloudserver/all";
			}else{  
				$("#tipscontent").html(data.message);
    ...
    function shutdownHost(){
    	jQuery.get(path + "/cloudserver/"+currentId+"/shutdown",function(data){
			if(data.status == "success"){   
	    		location.href = path + "/cloudserver/all";
			}else{  
				$("#tipscontent").html(data.message);
    ...
    function haltHost(){
    	jQuery.get(path + "/cloudserver/"+currentId+"/halt",function(data){
			if(data.status == "success"){   
	    		location.href = path + "/cloudserver/all";
			}else{  
				$("#tipscontent").html(data.message);
    ...
    function breakTenant(){
    	jQuery.get(path + "/cloudserver/"+currentId+"/breaktenant",function(data){
			if(data.status == "success"){   
	    		location.href = path + "/cloudserver/all";
			}else{  
				$("#tipscontent").html(data.message);
	        	...
	        	if(result.status == "fail"){ 
	        		  $("#tipscontent").html("登录超时，请重新登录");
	     		      $("#dia").click();
   		        			...
   		        			form.parsley('validate');
   		        			if(form.parsley('isValid')){  		        				
   		        				jQuery.get(path + "/cloudserver/"+currentId+"/"+$("#imageId").val()+"/start",function(data){
   		        					if(data.status == "success"){   
   		        			    		location.href = path + "/cloudserver/all";
   		        					}else{  
   		        						$("#tipscontent").html(data.message);
