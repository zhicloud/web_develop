...
function saveUserInfo(){
	...
	var urldata = "userIds="+array.join(",")+"&id="+tenantId;
	jQuery.ajax({
  	 	type: "POST",
  	 	async:false,
   		url: "<%=request.getContextPath() %>/tenant/setUser",
  		data: urldata,
    ...
    <div class="row">
        ...
        <div id="content" class="col-md-12">
          ...
          <div class="pageheader">
            ...
            <h2><i class="fa fa-chain-broken"></i> 配置租户【${tenant.name}】用户</h2>
          ...
          <div class="col-md-12">
                  ...
                  <span class="label label-default">未配置用户</span> 
                  <span class="label label-success" style="margin-left:52%;">已配置用户</span> 
                  </div>
                  <!-- /tile header -->
                      ...
                      <table  class="table table-datatable table-custom" id="unchoosetable" style="hegith:80%;font-size: 9px;width:99%;">
                        <thead>
                          <tr>
                            ...
                            <th class="no-sort">帐号</th>
                            <th class="no-sort">名称</th>
                          </tr>
                        </thead>
                        <tbody>
                        
					<c:forEach items="${outTenantUserList}" var="outUser">
                    		<tr class="odd gradeX">
						    <td>
									<div class="checkbox check-transparent">
									  <input type="checkbox" name="unchoosecheck" value="${outUser.billid }" id="${outUser.billid }">
									  <label for="${outUser.billid }"></label>
									</div>
                             </td>
                            <td class="cut">${outUser.usercount }</td>
                            <td class="cut">${outUser.displayname }</td>
                        </tr> 
                    	</c:forEach>                         
                        </tbody>
                      </table>
			...
			<div id="choosediv" class="table-responsive" style="width:40%;height:80%;;float: right;">
					...
					<c:forEach items="${tenantUserList}" var="user">
                    		<tr class="odd gradeX">
						    <td>
									<div class="checkbox check-transparent">
									  <input type="checkbox" name="choosecheck" value="${user.billid }" id="${user.billid }">
									  <label for="${user.billid }"></label>
									</div>
                             </td>
                            <td class="cut">${user.usercount }</td>
                            <td class="cut">${user.displayname }</td>
                        </tr> 
                        </tr> 
                    	</c:forEach>                         
                        </tbody>
