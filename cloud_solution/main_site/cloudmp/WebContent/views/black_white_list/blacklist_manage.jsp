...
<body class="bg-1">
                      ...
                      <table  class="table table-datatable table-custom" id="basicDataTable">
                        ...
                        <c:forEach items="${blacklistList}" var="blacklist">
                          ...
                          <tr class="odd gradeX">
						          <td>
									<div class="checkbox check-transparent">
									  <input type="checkbox" name="idcheck" value="${blacklist.ruleId}" id="${blacklist.ruleId}" >
									  <label for="${blacklist.ruleId}"></label>
									</div>
                                 </td>
                                  <td class="cut">${blacklist.ruleName}</td>
                                  <td class="cut">${blacklist.ruleIp}</td>
                                  <td class="cut">${blacklist.description}</td>
								  <td>
                                  		<div class="btn-group">
                                  ...
                                  <ul class="dropdown-menu" role="menu">
		                              <li><a href="javascript:void(0);" onclick="deleteBtn('${blacklist.ruleId }');" >删除</a></li>
		                              <li><a href="javascript:void(0);" onclick="updateBtn('${blacklist.ruleId }');" >修改</a></li>
                                  </ul>
                              </div>
                            </td>
                          </tr>
    ...
    $(function(){
	  ...
	  $("#add_blacklist").click(function(){
		  window.location.href = path+"/networkrule/blacklist/add";
 	  });
...
// 		        })
		        if(ids.length<1){
		        	$("#tipscontent").html("请选择要删除的黑名单");
					$("#dia").click();
		       	 	return;
		        } 
				curIds = ids;
				$("#confirmcontent").html("确定要删除所选黑名单吗？");
		    	$("#confirm_btn").attr("onclick","deleteMultBlacklist();");
		    	$("#con").click();
		  	}
	     });
    })
    ...
    function deleteBtn(id){
    	currentId = id;
    	$("#confirmcontent").html("确定要删除该黑名单吗？");
    	$("#confirm_btn").attr("onclick","deleteBlacklist();");
    	$("#con").click();
    }
    //修改黑名单
    function updateBtn(id){
    	window.location.href = path+"/networkrule/blacklist/"+id+"/update";
    }
    ...
    function deleteBlacklist(){
    	jQuery.get(path + "/networkrule/blacklist/"+currentId+"/delete",function(data){
    ...
    function deleteMultBlacklist(){
    	jQuery.ajax({
	        url: '<%=request.getContextPath()%>/networkrule/blacklist/deletemore',
	        type: 'post', 
	        dataType: 'json',
	        data:{ids:curIds},
	        timeout: 10000,
