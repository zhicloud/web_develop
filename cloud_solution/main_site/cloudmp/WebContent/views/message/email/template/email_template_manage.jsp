                    ...
                    <div class="col-md-12">
                        ...
                        <section class="tile color transparent-black">
                            ...
                            <ul class="nav nav-tabs tabdrop">
                                ...
                                <li class="active"><a href="#" data-toggle="tab" onclick="redirect('/message/email/template/list');">邮件模板管理</a></li>
                                <li class=""><a href="#" data-toggle="tab" onclick="redirect('/message/record/email/list');">邮件发送记录管理</a></li>
                            ...
                            <div class="tile-header">
                                <button type="button" class="btn btn-red delete" id="templateAdd">
                                    <i class="fa fa-plus"></i>
                                    <span> 新增邮件模板 </span>
                                </button>
		                    <button type="button" class="btn btn-green file-excel-o" onclick="exportData('/export/emailtemplatelistdata')">
                                ...
                                <div class="table-responsive">
                                    ...
                                    <table  class="table table-datatable table-custom table-sortable" id="basicDataTable">
                                        ...
                                        <tr>
                                            ...
                                            <th class="sortable sort-amount">邮件模板名称</th>
                                            <th class="sortable sort-amount">邮件服务器</th>
                                            <th class="sortable sort-amount">发送人邮箱</th>
                                            <th class="sortable sort-amount">收件人邮箱</th>
                                            <th class="sortable sort-amount">代码</th>
                                            <th class="sortable sort-amount">创建时间</th>
                                            <th class="sortable sort-amount">更新时间</th>
                                            <th class="no-sort">操作</th>
                                        </tr>
                                        </thead>
                                        <tbody>
                                        <c:forEach items="${email_template_list}" var="email_templates">
                                            <tr class="gradeX">
                                                <td>
                                                    <div class="checkbox check-transparent">
                                                        <input type="checkbox" value="${email_templates.id}" name="checkboxid" id="chck${email_templates.id}" >
                                                        <label for="chck${email_templates.id}"></label>
                                                    </div>
                                                </td>
                                                <td class="cut"><a hrep="javascript:void(0);" onclick="mod_template('${email_templates.id }');" style="color:#FAFAFA;cursor:pointer">${email_templates.name}</a></td>
                                                <td class="cut">${email_templates.configName}</td>
                                                <td class="cut">${email_templates.senderAddress}</td>
                                                <td class="cut">${email_templates.recipient}</td>
                                                <td class="cut">${email_templates.code}</td>
                                                <td class="cut"><fmt:formatDate
                                                        value="${email_templates.createTimeDate}"
                                                        pattern="yyyy-MM-dd HH:mm:ss"/></td>
                                                <td class="cut"><fmt:formatDate
                                                        value="${email_templates.modifiedTimeDate}"
                                                        pattern="yyyy-MM-dd HH:mm:ss"/></td>
                                                <td>
                                                    <div class="btn-group">
                                                        <button type="button"
                                                        ...
                                                        <ul class="dropdown-menu" role="menu">
                                                            <li><a href="#" onclick="mod_template('${email_templates.id }');">修改</a></li>
                                                            <li class="divider"></li>
                                                            <li><a href="#" onclick="del_template('${email_templates.id }');">删除</a></li>
                                                        </ul>
                                                    </div>
                                                </td>
                                            </tr>
                                            ...
                                            <div class="modal-body">
                                                <form role="form">
                                                    ...
                                                    <div class="form-group">
                                                        <label style="align:center;" id="confirmcontent">确定要删除该邮件模板吗？</label>
                                                    </div>
...
<script>
    ...
    $(function(){
        ...
        $("#templateAdd").click(function(){
            window.location.href = "${pageContext.request.contextPath}/message/email/template/add";
        });
        ...
        jQuery("#submit_oper").click(function(){
            ...
            if(option=="delete"){
                ...
                if(myids.length < 1){
                    $("#tipscontent").html("请选择要删除的邮件模板");
                    $("#dia").click();
                    return;
                }
                ids = myids;
                $("#confirmcontent").html("确定要删除所选邮件模板吗？");
    ...
    function mod_template(id) {
        window.location.href = "${pageContext.request.contextPath}/message/email/template/"+id+"/mod";
    }

    function del_template(id) {
        $("#confirmcontent").html("确定要删除该邮件模板吗？");
        curId = id;
    ...
    function deleteEmailConfig(){
        jQuery.get("${pageContext.request.contextPath}/message/email/template/"+curId+"/remove",
                function(data){
                    if(data.status=="success"){
                        location.href = "${pageContext.request.contextPath}/message/email/template/list";
                    }else{
    ...
    function deleteMultEmailConfig(){
        jQuery.ajax({
            url: '<%=request.getContextPath()%>/message/email/template/remove',
            type: 'post',
            dataType: 'json',
            data:{ids:ids},
            async: false,
