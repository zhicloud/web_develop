          ...
          <div class="col-md-12">
				  ...
				  <section class="tile color transparent-black">
                      ...
                      <ul class="nav nav-tabs tabdrop">
                          <li class="active"><a href="#" data-toggle="tab" onclick="redirect('/message/email/config/list');">邮件配置管理</a></li>
                  ...
                  <!-- tile header -->
                      <div class="tile-header">
                          <button type="button" class="btn btn-red delete" id="configAdd">
                              <i class="fa fa-plus"></i>
                              <span> 新增邮件配置 </span>
                          </button>
	                    <button type="button" class="btn btn-green file-excel-o" onclick="exportData('/export/emailconfiglistdata')">
                      ...
                      <table  class="table table-datatable table-custom table-sortable" id="basicDataTable">
                        <thead>
                          <tr>
                            ...
                            <div class="checkbox check-transparent">
                              ...
                              <th class="sortable sort-amount">邮件配置名称</th>
                              <th class="sortable sort-amount">发送人</th>
                        ...
                        <tbody>
                          <c:forEach items="${email_config_list}" var="email_configs">
                    		...
                    		<td>
								<div class="checkbox check-transparent">
								  <input type="checkbox" value="${email_configs.id}" name="checkboxid" id="chck${email_configs.id}" >
								  <label for="chck${email_configs.id}"></label>
								</div>
                                </td>
                                <td class="cut"><a hrep="javascript:void(0);" onclick="mod_config('${email_configs.id }');" style="color:#FAFAFA;cursor:pointer">${email_configs.name}</a></td>
                                <td class="cut">${email_configs.sender}</td>
                                <td class="cut">${email_configs.senderAddress}</td>
                                <td class="cut"><fmt:formatDate
                                        value="${email_configs.createTimeDate}"
                                        pattern="yyyy-MM-dd HH:mm:ss"/></td>
                                <td class="cut"><fmt:formatDate
                                        value="${email_configs.modifiedTimeDate}"
                                        pattern="yyyy-MM-dd HH:mm:ss"/></td>
                                <td>
                                    <div class="btn-group">
                                        <button type="button"
                                        ...
                                        <ul class="dropdown-menu" role="menu">
                                            <li><a href="#" onclick="mod_config('${email_configs.id }');">修改</a></li>
                                            <li class="divider"></li>
                                            <li><a href="#" onclick="del_config('${email_configs.id }');">删除</a></li>
                                        </ul>
                                    </div>
                                </td>
                        </tr>
                          ...
                          <div class="modal-body">
                            <form role="form">
                              ...
                              <div class="form-group">
                                <label style="align:center;" id="confirmcontent">确定要删除该邮件配置吗？</label>
                               </div>
    ...
    $(function(){
     ...
     $(function(){
         ...
         $("#configAdd").click(function(){
             window.location.href = "${pageContext.request.contextPath}/message/email/config/add";
         });
         ...
         jQuery("#submit_oper").click(function(){
             ...
             if(option=="delete"){
                 ...
                 if(myids.length < 1){
                     $("#tipscontent").html("请选择要删除的邮件配置");
                     $("#dia").click();
                     return;
                 }
                 ids = myids;
                 $("#confirmcontent").html("确定要删除所选邮件配置吗？");
        ...
        function mod_config(id) {
            window.location.href = "${pageContext.request.contextPath}/message/email/config/"+id+"/mod";
        }

        function del_config(id) {
            $("#confirmcontent").html("确定要删除该邮件配置吗？");
            curId = id;
        ...
        function deleteEmailConfig(){
            jQuery.get("${pageContext.request.contextPath}/message/email/config/"+curId+"/remove",
                    function(data){
                        if(data.status=="success"){
                            location.href = "${pageContext.request.contextPath}/message/email/config/list";
                        }else{
        ...
        function deleteMultEmailConfig(){
            jQuery.ajax({
                url: '<%=request.getContextPath()%>/message/email/config/remove',
                type: 'post',
                dataType: 'json',
                data:{ids:ids},
                async: false,
