...
public class HttpGatewayChannel
{
     ...
     * @param option: uint array 0否/1是,[是否使用磁盘镜像创建,是否需要数据盘,是否自动启动，是否支持本地备份，是否支持usb扩展]
     ...
     * @param inbound_bandwidth: uint 入口带宽，单位：字节
     * @param outbound_bandwidth: uint 出口带宽，单位：字节
     * @param path: ip san : iqn@ip:port
     * @param crypt: 磁盘连接校验信息
     */
    public synchronized JSONObject hostCreate(
        String name, String pool, Integer cpu_count, BigInteger memory, Integer[] option, String image,
        BigInteger[] disk_volume, Integer[] port, String user, String group, String display, String authentication,
    ...
    ) throws MalformedURLException, IOException
    {
        try
        {
            ...
            JSONObject result = helper.hostCreate( name, pool, cpu_count, memory, option, image,
                disk_volume, port, user, group, display, authentication,
                network, inbound_bandwidth, outbound_bandwidth, path, crypt, maxIops, cpuPriority);
            if( HttpGatewayReturnCode.SESSION_NOT_FOUND.equals(HttpGatewayResponseHelper.getReturnCode(result)) )
            {
                helper = null;
            }
    ...
    public synchronized JSONObject hostReset(String uuid, String callback) throws MalformedURLException, IOException
    {
        try
        {
            logger.debug("HttpGatewayChannel.hostReset() > ["+Thread.currentThread().getId()+"] uuid:["+uuid+"]");
            checkSessionRefresh();
            JSONObject result = helper.hostReset(uuid, callback);
            if( HttpGatewayReturnCode.SESSION_NOT_FOUND.equals(HttpGatewayResponseHelper.getReturnCode(result)) )
            {
                helper = null;
            }
...
//  }
    ...
    public synchronized JSONObject computePoolDetail(String uuid, String callback) throws MalformedURLException, IOException
    {
        try
        {
            checkSessionRefresh();
            JSONObject result = helper.computePoolDetail(uuid, callback);
            if( HttpGatewayReturnCode.SESSION_NOT_FOUND.equals(HttpGatewayResponseHelper.getReturnCode(result)) )
            {
                helper = null;
            }
	...
	public synchronized JSONObject computePoolCreate(String name, int network_type, String network, int disk_type, String disk_source,Integer[] mode, String path, String crypt) throws MalformedURLException, IOException
	{
		try
		{
			checkSessionRefresh();
			JSONObject result = helper.computePoolCreate(name, network_type, network, disk_type, disk_source, mode,  path,  crypt);
			if( HttpGatewayReturnCode.SESSION_NOT_FOUND.equals(HttpGatewayResponseHelper.getReturnCode(result)) )
			{
				helper = null;
			}
    ...
    public synchronized JSONObject computePoolCreate(String name, Integer networkType, String network, Integer diskType, String diskSource, Integer[] mode, String path, String crypt, String callback) throws MalformedURLException, IOException
    {
        try
        {
            checkSessionRefresh();
            JSONObject result = helper.computePoolCreate(name, networkType, network, diskType, diskSource, mode, path, crypt, callback);
            if( HttpGatewayReturnCode.SESSION_NOT_FOUND.equals(HttpGatewayResponseHelper.getReturnCode(result)) )
            {
                helper = null;
            }
    ...
    public synchronized JSONObject computePoolModify(String uuid, String name, int networkType, String network, int diskType, String diskSource, Integer[] mode, int option,String path,String crypt, String callback) throws MalformedURLException, IOException
    {
        try
        {
            checkSessionRefresh();
            JSONObject result = helper.computePoolModify(uuid, name, networkType, network, diskType, diskSource, mode, option, path, crypt, callback);
            if( HttpGatewayReturnCode.SESSION_NOT_FOUND.equals(HttpGatewayResponseHelper.getReturnCode(result)) )
            {
                helper = null;
            }
    ...
    public synchronized JSONObject serviceEnable(String target) throws MalformedURLException, IOException
    {
        try
        {
            checkSessionRefresh();
            JSONObject result = helper.serviceEnable(target);
            if( HttpGatewayReturnCode.SESSION_NOT_FOUND.equals(HttpGatewayResponseHelper.getReturnCode(result)) )
            {
                helper = null;
            }
    ...
    public synchronized JSONObject serviceDisable(String target) throws MalformedURLException, IOException
    {
        try
        {
            checkSessionRefresh();
            JSONObject result = helper.serviceDisable(target);
            if( HttpGatewayReturnCode.SESSION_NOT_FOUND.equals(HttpGatewayResponseHelper.getReturnCode(result)) )
            {
                helper = null;
            }
    ...
    public synchronized JSONObject hostFlushDiskImage(String uuid, int disk, int mode, String image, String callback) throws MalformedURLException, IOException {
        try {
            this.checkSessionRefresh();
            JSONObject result = helper.hostFlushDiskImage(uuid, disk, mode, image, callback);
    ...
    public synchronized JSONObject hostBackup(String uuid, int mode, int disk, String callback) throws MalformedURLException, IOException {
        try {
            this.checkSessionRefresh();
            JSONObject result = helper.hostBackup(uuid, mode, disk, callback);
    ...
    public synchronized JSONObject hostResume(String uuid, int mode, int disk, String callback) throws MalformedURLException, IOException {
        try {
            this.checkSessionRefresh();
            JSONObject result = helper.hostResume(uuid, mode, disk, callback);
    ...
    public synchronized JSONObject snapshotQuery(int type,String uuid, int mode, int index, String callback) throws MalformedURLException, IOException {
        try {
            this.checkSessionRefresh();
            JSONObject result = helper.snapshotQuery(type,uuid,mode,index, callback);
            if (HttpGatewayReturnCode.SESSION_NOT_FOUND.equals(HttpGatewayResponseHelper.getReturnCode(result))) {
                helper = null;
            }         
            return result;
    ...
    public synchronized JSONObject snapshotDel(int type,String uuid, int mode, String snapshotId, String callback) throws MalformedURLException, IOException {
        try {
            this.checkSessionRefresh();
            JSONObject result = helper.snapshotDel(type,uuid,mode,snapshotId, callback);
            if (HttpGatewayReturnCode.SESSION_NOT_FOUND.equals(HttpGatewayResponseHelper.getReturnCode(result))) {
                helper = null;
            }         
            return result;
    ...
    public synchronized JSONObject snapshotCreate(int type,String uuid, int mode, String index, String callback) throws MalformedURLException, IOException {
        try {
            this.checkSessionRefresh();
            JSONObject result = helper.snapshotCreate(type,uuid,mode,index, callback);
            if (HttpGatewayReturnCode.SESSION_NOT_FOUND.equals(HttpGatewayResponseHelper.getReturnCode(result))) {
                helper = null;
            }         
            return result;
    ...
    public synchronized JSONObject snapshotResume(int type,String uuid, int mode, String snapshotId, String callback) throws MalformedURLException, IOException {
        try {
            this.checkSessionRefresh();
            JSONObject result = helper.snapshotDel(type,uuid,mode,snapshotId, callback);
            if (HttpGatewayReturnCode.SESSION_NOT_FOUND.equals(HttpGatewayResponseHelper.getReturnCode(result))) {
    ...
    public synchronized JSONObject hostQueryBackup(String uuid, String callback) throws MalformedURLException, IOException {
        try {
            this.checkSessionRefresh();
            JSONObject result = helper.hostQueryBackup(uuid, callback);
    ...
    public synchronized JSONObject ruleAdd(String target, int mode, String[] ip, Integer[] port, String callback) throws MalformedURLException, IOException {
        try
        {
            checkSessionRefresh();
            JSONObject result = helper.ruleAdd(target, mode, ip, port, callback);
            if( HttpGatewayReturnCode.SESSION_NOT_FOUND.equals(HttpGatewayResponseHelper.getReturnCode(result)))
            {
                helper = null;
            }
    ...
    public synchronized JSONObject ruleRemove(String target, int mode, String[] ip, Integer[] port, String callback) throws MalformedURLException, IOException {
        try
        {
            checkSessionRefresh();
            JSONObject result = helper.ruleRemove(target, mode, ip, port, callback);
            if( HttpGatewayReturnCode.SESSION_NOT_FOUND.equals(HttpGatewayResponseHelper.getReturnCode(result)))
            {
                helper = null;
            }
    ...
    public synchronized JSONObject ruleQuery(String target, String callback) throws MalformedURLException, IOException {
        try
        {
            checkSessionRefresh();
            JSONObject result = helper.ruleQuery(target, callback);
            if( HttpGatewayReturnCode.SESSION_NOT_FOUND.equals(HttpGatewayResponseHelper.getReturnCode(result)))
            {
                helper = null;
            }
    ...
    public synchronized JSONObject startMonitor(int level, String[] target, String callback)
            throws MalformedURLException, IOException {
        try {
            checkSessionRefresh();
            JSONObject result = helper.startMonitor(level, target, callback);
            if (HttpGatewayReturnCode.SESSION_NOT_FOUND.equals(HttpGatewayResponseHelper.getReturnCode(result))) {
                helper = null;
            }
            return result;
    ...
    public synchronized JSONObject monitorHeartbeat(int task) throws MalformedURLException, IOException {
        try {
            checkSessionRefresh();
            JSONObject result = helper.monitorHeartbeat(task);
            if (HttpGatewayReturnCode.SESSION_NOT_FOUND.equals(HttpGatewayResponseHelper.getReturnCode(result))) {
                helper = null;
            }
            return result;
    ...
    public synchronized JSONObject stopMonitor(int task, String callback) throws MalformedURLException, IOException {
        try {
            checkSessionRefresh();
            JSONObject result = helper.stopMonitor(task, callback);
            if (HttpGatewayReturnCode.SESSION_NOT_FOUND.equals(HttpGatewayResponseHelper.getReturnCode(result))) {
                helper = null;
            }
            return result;
	...
	public synchronized JSONObject serverQueryStorageDevice(int level, String target, int diskType, String callback) throws MalformedURLException, IOException {
		try
		{
			checkSessionRefresh();
			JSONObject result = helper.serverQueryStorageDevice(level, target, diskType, callback);
			if( HttpGatewayReturnCode.SESSION_NOT_FOUND.equals(HttpGatewayResponseHelper.getReturnCode(result)))
			{
				helper = null;
			}
	...
	public synchronized JSONObject serverAddStorageDevice(int level, String target, int diskType, Integer[] index, String name, String path, String crypt, String callback) throws MalformedURLException, IOException {
		try
		{
			checkSessionRefresh();
			JSONObject result = helper.serverAddStorageDevice(level, target, diskType, index, name, path, crypt, callback);
			if( HttpGatewayReturnCode.SESSION_NOT_FOUND.equals(HttpGatewayResponseHelper.getReturnCode(result)))
			{
				helper = null;
			}
	...
	public synchronized JSONObject serverRemoveStorageDevice(int level, String target, int diskType, int index, String callback) throws MalformedURLException, IOException {
		try
		{
			checkSessionRefresh();
			JSONObject result = helper.serverRemoveStorageDevice(level, target, diskType, index, callback);
			if( HttpGatewayReturnCode.SESSION_NOT_FOUND.equals(HttpGatewayResponseHelper.getReturnCode(result)))
			{
				helper = null;
			}
	...
	public synchronized JSONObject serverEnableStorageDevice(int level, String target, int diskType, int index, String callback) throws MalformedURLException, IOException {
		try
		{
			checkSessionRefresh();
			JSONObject result = helper.serverEnableStorageDevice(level, target, diskType, index, callback);
			if( HttpGatewayReturnCode.SESSION_NOT_FOUND.equals(HttpGatewayResponseHelper.getReturnCode(result)))
			{
				helper = null;
			}
	...
	public synchronized JSONObject serverDisableStorageDevice(int level, String target, int diskType, int index, String callback) throws MalformedURLException, IOException {
		try
		{
			checkSessionRefresh();
			JSONObject result = helper.serverDisableStorageDevice(level, target, diskType, index, callback);
			if( HttpGatewayReturnCode.SESSION_NOT_FOUND.equals(HttpGatewayResponseHelper.getReturnCode(result)))
			{
				helper = null;
			}
    ...
    public synchronized JSONObject serviceModify(int type, String target, int diskType, String diskSource, String crypt, String callback) throws MalformedURLException, IOException {
        try
        {
            checkSessionRefresh();
            JSONObject result = helper.serviceModify(type, target, diskType, diskSource, crypt, callback);
            if( HttpGatewayReturnCode.SESSION_NOT_FOUND.equals(HttpGatewayResponseHelper.getReturnCode(result)))
            {
                helper = null;
            }
    ...
    public synchronized JSONObject networkDeviceQuery(int level, String target, int type, String callback) throws MalformedURLException, IOException {
        try
        {
            checkSessionRefresh();
            JSONObject result = helper.networkDeviceQuery(level, target, type, callback);
            if( HttpGatewayReturnCode.SESSION_NOT_FOUND.equals(HttpGatewayResponseHelper.getReturnCode(result)))
            {
                helper = null;
            }
    ...
    public synchronized JSONObject networkDeviceAttach(int level, String target, int type, int index, int networkId, String callback) throws MalformedURLException, IOException {
        try
        {
            checkSessionRefresh();
            JSONObject result = helper.networkDeviceAttach(level, target, type, index, networkId, callback);
            if( HttpGatewayReturnCode.SESSION_NOT_FOUND.equals(HttpGatewayResponseHelper.getReturnCode(result)))
            {
                helper = null;
            }
    ...
    public synchronized JSONObject networkDeviceDetach(int level, String target, int type, int index, int networkId, String callback) throws MalformedURLException, IOException {
        try
        {
            checkSessionRefresh();
            JSONObject result = helper.networkDeviceDetach(level, target, type, index, networkId, callback);
            if( HttpGatewayReturnCode.SESSION_NOT_FOUND.equals(HttpGatewayResponseHelper.getReturnCode(result)))
            {
                helper = null;
            }
    ...
    public synchronized JSONObject bondGroupQuery(int level, String target, int type, String callback) throws MalformedURLException, IOException {
        try
        {
            checkSessionRefresh();
            JSONObject result = helper.bondGroupQuery(level, target, type, callback);
            if( HttpGatewayReturnCode.SESSION_NOT_FOUND.equals(HttpGatewayResponseHelper.getReturnCode(result)))
            {
                helper = null;
            }
    ...
    public synchronized JSONObject bondGroupCreate(int level, String target, int type, String name, String callback) throws MalformedURLException, IOException {
        try
        {
            checkSessionRefresh();
            JSONObject result = helper.bondGroupCreate(level, target, type, name, callback);
            if( HttpGatewayReturnCode.SESSION_NOT_FOUND.equals(HttpGatewayResponseHelper.getReturnCode(result)))
            {
                helper = null;
            }
    ...
    public synchronized JSONObject bondGroupModify(int level, String target, int type, String name, int index, int mode, String ip, String netmask, String callback) throws MalformedURLException, IOException {
        try
        {
            checkSessionRefresh();
            JSONObject result = helper.bondGroupModify(level, target, type, name, index, mode, ip, netmask, callback);
            if( HttpGatewayReturnCode.SESSION_NOT_FOUND.equals(HttpGatewayResponseHelper.getReturnCode(result)))
            {
                helper = null;
            }
    ...
    public synchronized JSONObject bondGroupDelete(int level, String target, int type, int index, String callback) throws MalformedURLException, IOException {
        try
        {
            checkSessionRefresh();
            JSONObject result = helper.bondGroupDelete(level, target, type, index, callback);
            if( HttpGatewayReturnCode.SESSION_NOT_FOUND.equals(HttpGatewayResponseHelper.getReturnCode(result)))
            {
                helper = null;
            }
