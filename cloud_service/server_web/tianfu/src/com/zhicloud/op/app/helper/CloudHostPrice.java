	...
	static {
		...
		if(cpuList!=null && cpuList.size()>0){
			for(PackagePriceVO packagePrice : cpuList){
				...
				putCpuMonthlyPrice(packagePrice.getRegion(),packagePrice.getPriceStatus(),packagePrice.getCpuCore(), packagePrice.getPrice());
			}
		}
	...
	static {
		...
		if(memoryList!=null && memoryList.size()>0){
			for(PackagePriceVO packagePrice : memoryList){
				putMemoryMonthlyPrice(packagePrice.getRegion(),packagePrice.getPriceStatus(),CapacityUtil.toGBValue(packagePrice.getMemory(),0,BigDecimal.ROUND_UP).intValue(), packagePrice.getPrice());
			}
		}
...
//		putMemoryMonthlyPrice(0,64, new BigDecimal(2236));
	...
	public static BigDecimal getDataDiskMonthlyPrice(Integer region,Integer priceStatus, Double dataDiskOfGB)
	{
		...
		if(priceStatus == 2){
			if(region == 4){
				return DATA_DISK_10GB_MONTHLY_PRICE_FOR_HONGKANG_VPC.multiply(new BigDecimal(dataDiskOfGB));
			}else if(region == 2){
				return DATA_DISK_10GB_MONTHLY_PRICE_FOR_CHENGDU_VPC.multiply(new BigDecimal(dataDiskOfGB));
			}else{
				return DATA_DISK_10GB_MONTHLY_PRICE_FOR_OTHERS_VPC.multiply(new BigDecimal(dataDiskOfGB));
			}
		}else{
			if(region == 4){
...
//	}
	...
	public static BigDecimal getBandwidthMonthlyPrice(Integer region,Integer priceStatus, Integer bandwidthOfMbps)
	{
		...
		if(priceStatus == 2){
			...
			{
				if(region==4){
					return LESS_THAN_5_HK_VPC.multiply(new BigDecimal(bandwidthOfMbps));
				}else if(region==2){
					return LESS_THAN_5_CD_VPC.multiply(new BigDecimal(bandwidthOfMbps));
				}else{
					return LESS_THAN_5_OTHER_VPC.multiply(new BigDecimal(bandwidthOfMbps));
				}
			}
			else
			{
				if(region == 4){
					return BASE_PRICE_5_HK_VPC.add( new BigDecimal(bandwidthOfMbps-5).multiply(BANDWIDTH_MONTHLY_PRICE_PER_1MBPS_4_VPC) );
				}else if(region == 2){
					return BASE_PRICE_5_CD_VPC.add( new BigDecimal(bandwidthOfMbps-5).multiply(BANDWIDTH_MONTHLY_PRICE_PER_1MBPS_2_VPC) );
				}else{
					return BASE_PRICE_5_OTHER_VPC.add( new BigDecimal(bandwidthOfMbps-5).multiply(BANDWIDTH_MONTHLY_PRICE_PER_1MBPS_VPC) );
				}
			}
		}else{
