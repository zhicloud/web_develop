...
$(document).ready(function(){
	...
	$("#one_cloud_host .box").hover(function(evt){
		...
		self.display_name = $(this).attr("display_name");
	});
	// 查看详情
	...
	// 绑定域名
	$("#one_cloud_host .bind_domain").click(function(evt){
		var host_id = self.cloudHostId;
		var display_name = self.display_name;
		var port = 80;
		ajax.remoteCall("bean://cloudHostService:checkPortIsOpened",
				[host_id, port], 
				function(reply) {
					if(reply.result.status == "fail") {
						top.$.messager.alert("警告", "该云主机暂未开放80端口", "warning");
						return false;
					}else {
						window.location.href="<%=request.getContextPath()%>/bean/page.do?userType=<%=userType%>&bean=cloudHostService&method=manageDomainPage&host_id="+encodeURIComponent(host_id)+"&display_name="+encodeURIComponent(display_name);
					}
				}
			);
...
<div class="page">
  <div class="pageleft">
    ...
    <div class="main">
      <div class="titlebar">
        <div class="tabbar l"><a id="all_cloud_host" name="cloud_host" region="0" href="javascript:void(0);" class="active">全部</a>　｜　<a id="chengdu_cloud_host" region="2" name="cloud_host" href="javascript:void(0);">成都</a>　｜　<a id="hongkang_cloud_host" region="4" name="cloud_host" href="javascript:void(0);">香港</a> &nbsp;&nbsp; | &nbsp;&nbsp; <a id="guangzhou_cloud_host" region="1" name="cloud_host" href="javascript:void(0);">广州</a></div>
      ...
      <div id="one_cloud_host" style="width:720px;padding: 6px 0 0 0; margin:0 auto;">
        <table id="my_warehouse_datagrid" class="easyui-datagrid" data-options="border:false,showHeader:false,scrollbarSize:0,width:720">
          ...
          %>
            <tr>
              <td><div class="box" cloudHostId="<%=cloudHost.getId()%>" ip="<%=cloudHost.getOuterIp() %>" password="<%=cloudHost.getPassword()%>" display_name="<%=cloudHost.getDisplayName()%>">
...
<%--                           <input id="t1<%=cloudHost.getId() %>" class="host_name" onblur="changeHostName()" maxlength="100" name="" type="text" value="<%=cloudHost.getDisplayName()%>"/> --%>
                    ...
                    <%if(cloudHost.getRegion()!=null && cloudHost.getRegion()==2){ %>
                    	<div class="div4 l"><div class="l"><img src="<%=request.getContextPath() %>/image/icon_ip.png" width="20" height="20" alt=" " style="vertical-align:middle" title="外网IP:<%=cloudHost.getOuterIp()==null ? "" : cloudHost.getOuterIp()%> 端口:<%=cloudHost.getOuterPort()==null ? "" : cloudHost.getOuterPort()%> "/> <%=StringUtil.isBlank(cloudHost.getOuterIp()) ? "" : cloudHost.getOuterIp() + ":" + (cloudHost.getOuterPort()==null ? "" : cloudHost.getOuterPort())%></div>
...
<!-- 		                  <a href="javascript:void(0);" class="graybutton l enter_my_cloud_host"><div class="r">进入主机</div><div class="btn10 l">&nbsp;</div></a> -->
		                  ...
		                  <%if(cloudHost.getType()==2){ %>
		                    ...
		                    <%if(cloudHost.getRegion()!=null && cloudHost.getRegion()==2){ %>
		                 	<a id="modifiy_port<%=cloudHost.getId() %>" href="javascript:void(0);" class="btn btn13 l modify-port" title="端口详情" modifyPort="1">&nbsp;</a>
 		                 	<a id="bind<%=cloudHost.getId() %>" href="javascript:void(0);" class="btn btn19 l bind_domain" title="绑定域名" >&nbsp;</a>			   
		                  <%}}else{ %>
		                  	<a id="outOfService<%=cloudHost.getId() %>" href="javascript:void(0);" class="btn newbtn5 l out-of-service" title="停用" outofservice="1">&nbsp;</a>
					...
					<%}else if(cloudHost.getRunningStatus() == AppConstant.CLOUD_HOST_RUNNING_STATUS_SHUTTING_DOWN || cloudHost.getRunningStatus() == AppConstant.CLOUD_HOST_RUNNING_STATUS_STARTING) {%>
						<div class="listright l" name="a<%=cloudHost.getId()%>">
		                  <a id="start<%=cloudHost.getId() %>" href="javascript:void(0);" class="btn btn1 alpha l launch_host" title="开机" startHost="2">&nbsp;</a>
             ...
             <%--<tr>
              <td><div class="box">
                  <div class="listleft l">
                    <div class="div1 l">
                      <div class="listicon">
                        <a href="javascript:void(0);" class="btn centos" title="CentOS 6.4 32bit">&nbsp;</a>
                        <div class="text">
            ...
            <tr>
              <td><div class="box">
                  <div class="listleft l">
                    <div class="div1 l">
                      <div class="listicon">
                        <a href="javascript:void(0);" class="btn windows" title="Windows 2000 32bit">&nbsp;</a>
                        <div class="text">
                          <input id="t1" name="" type="text" value="Host_1"/>
                          <label for="t1" class="smalllabel l" style="cursor:pinter; background:url(image/icon_edit.png) no-repeat center right"></label>
                        </div>
