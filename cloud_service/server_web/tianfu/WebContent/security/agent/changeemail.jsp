...
function confirmationChange(){
	...
	var formData = $.formToBean(change_email_choice_option);
	ajax.remoteCall("bean://agentService:updateBaseInfoEmail",
		[formData],
		function(reply)
		{
			if( reply.status=="exception" )
...
function sendEmailMessage(obj,second){
	...
	if(second==55){
		var formData = $.formToBean(change_email_choice_option);
		ajax.remoteCall("bean://agentService:updateBaseInfoEmailSendCode",
			[formData],
			function(reply)
			{
				if( reply.status=="exception" )
...
function sendEmailMessageAgain(obj,second){
	...
	if(second==58){
		var formData = $.formToBean(change_email_choice_option);
		ajax.remoteCall("bean://agentService:updateBaseInfoEmailSendCodeAgain",
			[formData],
			function(reply)
			{
				if( reply.status=="exception" )
