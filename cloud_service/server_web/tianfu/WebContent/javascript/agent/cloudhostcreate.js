...
$(document).ready(function(){ 
	...
	$("#goRechargeRightNow").click(function(){
		var chkObjs = document.getElementsByName("total_fee");
		var total_fee = "";
    	for(var i=0;i<chkObjs.length;i++){
    		if(chkObjs[i].checked){
    			total_fee = chkObjs[i].value; 
    			break;
    		}
    	}
    	if(total_fee==""){
	...
	$("#finish").click(function(){
		window.location.href = a+"/bean/page.do?userType=3&bean=terminalUserService&method=viewItemPage&terminalUserId="+userId;
	});  
	...
	$("#rh").blur(function(){ 
		if($(this).val()!=''){			
			$("#rh1").removeAttr("checked");
			$("#rh1").next("label").removeClass("checked");
			$("#rh2").removeAttr("checked");
			$("#rh2").next("label").removeClass("checked");
			$("#rh3").removeAttr("checked");
			$("#rh3").next("label").removeClass("checked");  
...
//    } 
    ...
    var info ='<div class="l" style="width:120px;"><span class="bluebutton" style="width:105px;margin:0 15px 15px 0">主机名称</span></div>'+
            ...
            '<input id="t1" name="displayName" type="text" value="'+hostName+'" class="einput"/>  <div class="l" id="displayName_">'+hostName+'</div>  </div>'+
...
function showDescription(description){
	if(description!=''){		
		$("#isotitle").empty();
		$("#isotitle").html("版本信息："+description);
	}else{
		var chkObjs = document.getElementsByName("sysImageId"); 
	    for(var i=0;i<chkObjs.length;i++){
	        if(chkObjs[i].checked==true){  
	        	$("#isotitle").empty();
	        	$("#isotitle").html("版本信息："+$(chkObjs[i]).attr("imgdescription"));
	        	break;
	        }
	    } 
	}
...
function getDescriptionAndPrice(){
    ...
    monthlyprice = (monthlyprice*percent).toFixed(2);
...
function getLoginInfo(username,message,id){
	...
	$("#my_clouduan").click(function(){
		window.location.href = a+"/agent.do";
	}); 
	getHostName();
	getDescriptionAndPrice();
	showDescription();
...
function checkRegion(){
    ...
    if(region_val=="1"){
    	...
    	if(childNum==0){
    		...
    		$("#getImage").attr("onclick","getComfirmInfo();nextstep();");    		
    	}
    }else if(region_val=="4"){
    	...
    	if(childNum==0){
    		...
    		$("#isotitle").html("版本信息："+$("#image_region_2").find(":first-child").attr("imgdescription"));
     		$("#getImage").attr("class","bluebutton r");    		
    		$("#getImage").attr("onclick","getComfirmInfo();getDefaultPort();checkPortsCount();nextstep();");    		
    	}
...
function addPort(){
	...
	if(newPort == 0||newPort > 65535){
	 ...
	 var html = '<tr>'+
               ...
               '<input name="ports" type="checkbox" checked="checked" onclick="checkPortsCount();" value="&'+protocol+'&'+newPort+'" />'+
               '     </div></td>'+
...
function getDefaultPort(){
    ...
    if(region_val=="2"){ 
	     ...
	     if(sysImageName.indexOf("windows")!=-1 || sysImageName.indexOf("Windows")!=-1){
             ...
             '  <td style="width:28px"><div class="datagrid-cell">'+
             '  <input name="ports" type="checkbox" disabled="disabled" checked="checked" onclick="checkPortsCount();" value="FTP&1&21,30000-33003" />'+
             '  </div></td>'+
             ...
             '  <td style="width:28px"><div class="datagrid-cell">'+
             '  <input name="ports" type="checkbox" disabled="disabled" checked="checked" onclick="checkPortsCount();" value="RDP&1&3389" />'+
             '  </div></td>'+
	    	 ...
	    	 var html = '<tr class="nochange">'+
             '  <td style="width:28px"><div class="datagrid-cell">'+
             '  <input name="ports" type="checkbox" disabled="disabled" checked="checked" onclick="checkPortsCount();" value="FTP&1&21,30000-30003" />'+
             '  </div></td>'+
             '  <td style="width:231px"><div class="datagrid-cell">FTP</div></td>'+
             '  <td style="width:231px"><div class="datagrid-cell">TCP</div></td>'+
             '  <td style="width:230px"><div class="datagrid-cell">21,30000-30003</div></td>'+
             ...
             html = html +  '<tr class="nochange">'+
             '  <td style="width:28px"><div class="datagrid-cell">'+
             '  <input name="ports" type="checkbox" disabled="disabled" checked="checked" onclick="checkPortsCount();" value="SSH&1&22" />'+
             '  </div></td>'+
             ...
             html = html +  '<tr class="nochange">'+
             '  <td style="width:28px"><div class="datagrid-cell">'+
             '  <input name="ports" type="checkbox" disabled="disabled" checked="checked" onclick="checkPortsCount();" value="主机面板&1&10000" />'+
             '  </div></td>'+
...
function getHostName(){  
	...
	ajax.remoteCall("bean://cloudHostService:getHostNameForAgentCreate", 
		[userId,region_val],
		function(reply) {
			if (reply.status == "exception") {
				if(reply.errorCode=="RMC_1"){
