...
public class AlertServiceProcessor extends AbstractAlertServiceProcessor {
		...
		if(firstTime == 0){
			...
			if(serviceList.size() > 0){
				...
				for(ServiceInfoExt sie : serviceList){
					logger.info(sie.getRegion()+"--(1Min)Malfunction monitoring：the server ["+sie.getName()+"] status is "+sie.getStatus()+"and fault type is "+sie.getType());
			...
			for(Long time : keys){
				if(System.currentTimeMillis()-time >= 1000*60*60){
					...
					for (ServiceInfoExt server : curList) {
						logger.info(server.getRegion()+"--(60Min)Malfunction monitoring：the server ["+server.getName()+"] status is "+server.getStatus()+"and fault type is "+server.getType());
  ...
  public synchronized void recoverEvent(List<ServiceInfoExt> serviceList) {
		...
		for(ServiceInfoExt sie : serviceList){
			...
			if(t!=null){
				...
				for(int i=0;i<l.size();i++){
					//判断故障是否对应，对应则从故障组中删除
					if((l.get(i).getCheckName()+l.get(i).getRegion()).equals(sie.getCheckName()+sie.getRegion())){
						l.remove(i);
					}
				}
		...
		for (ServiceInfoExt server : serviceList) {
			logger.info("Restore malfunction monitoring：the server ["+server.getName()+"] status is "+server.getStatus()+"and fault type is "+server.getType());
			int region = server.getRegion();
			int status = server.getStatus();
			int type   = server.getType();
			String reg = "";
