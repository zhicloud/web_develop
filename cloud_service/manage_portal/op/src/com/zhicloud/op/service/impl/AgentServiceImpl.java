...
public class AgentServiceImpl extends BeanDirectCallableDefaultImpl implements AgentService
{
    ...
    {
        logger.debug("AgentServiceImpl.userPage()");
...
/*        SysGroupMapper sysGroupMapper = this.sqlSession.getMapper(SysGroupMapper.class);
        ...
        return "/security/operator/agent_user_manage.jsp";
    }	
	...
	public String modPage(HttpServletRequest request, HttpServletResponse response)
	{
		...
		List<MarkVO> markList = markMapper.getAll();
		request.setAttribute("markList", markList);
		MarkBindUserVO bindUserVO = markBindUserMapper.queryByUserId(agentId);
		if(bindUserVO!=null){
...
//			}
			...
			// 判断该账号和昵称是否已经存在
            AgentVO agent = agentMapper.getAgentByName(name);
            if (agent != null) {
                return new MethodResult(MethodResult.FAIL, "name-" + name + "已存在");
            }
            
            agent = agentMapper.getAgentByAccount(email);
            if (agent != null) {
                return new MethodResult(MethodResult.FAIL, "email-" + email + "已存在");
            }
            agent = agentMapper.getAgentByPhone(phone);
            if (agent != null) {
                return new MethodResult(MethodResult.FAIL, "phone-" + phone + "已存在");
            }
...
//			}
	...
	public MethodResult deleteAgentByIds(List<?> agentIds)
	{
		...
		try
		{
			...
			//删除与标签的关联
			MarkBindUserMapper markBindUserMapper = this.sqlSession.getMapper(MarkBindUserMapper.class);
			markBindUserMapper.deleteByUserIds((List<String>)agentIds);
...
//							System.out.println("-----fail-----:"+state);
  ...
  public MethodResult updateBaseInfoPhoneSendMessage(Map<String, String> parameter)
    ...
    @Override
    public List<AgentVO> getAgentDataByCondition(HttpServletRequest request, HttpServletResponse response) {
        logger.debug("AgentServiceImpl.getAgentDataByCondition()");
        try {
            response.setCharacterEncoding("utf-8");
            response.setContentType("text/json; charset=utf-8");
            // 获取参数
            ...
            logger.error("AgentServiceImpl.getAgentDataByCondition()", e);
            throw new AppException("查询失败");
        }
    }
}
