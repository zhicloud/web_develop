...
public class HttpGatewayChannelExt extends HttpGatewayChannel {
	...
	public JSONObject getDefaultStoragePool() throws MalformedURLException, IOException {
		JSONObject result = this.storagePoolQuery();

		if (HttpGatewayResponseHelper.isSuccess(result)) {
			List<JSONObject> storagePools = (List<JSONObject>) result.get("storagePools");
			for (JSONObject storagePool : storagePools) {
				if ("default".equalsIgnoreCase(storagePool.getString("name"))) {
					return storagePool;
				}
			}
		}

		logger.debug("found no default storage pool");
		return null;
	}
