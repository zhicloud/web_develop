...
public class CheckServerRoomsListener implements ServletContextListener {
    ...
    /** ============================================= **/
    // 监控服务接口
    private MonitorService monitorService = CoreSpringContextManager.getMonitorService();
    // 规则服务接口
    private SysWarnService sysWarnService = CoreSpringContextManager.getSysWarnService();
        ...
        if (ERROR_PERIOD_MIN > 0) {
            ...
            if (error_notify_type == fixtime && error_on_off == yes) {
                timer.schedule(new TimerTask() {
                    ...
                    public void run() {
                        try {
                            // 由于不能更具服务器取得云主机信息，暂时只汇总服务器的相关数据
    ...
    public void updateMemoryData() throws Exception {
        ...
        for (RegionData regionData : regionDatas) {
            ...
            for (int i = 0; i < rooms.size(); i++) {
                ...
                // 将上级ID放入JSON，用于页面查询
                room.put("areaid", regionData.getId());
                // 更新屏蔽信息
                ...
                for (int j = 0; j < racks.size(); j++) {
                    ...
                    // 将上级ID放入JSON，用于页面查询
                    rack.put("areaid", regionData.getId());
                    rack.put("roomid", JSONLibUtil.getString(room, "uuid"));
                    // 更新屏蔽信息
                    MonitorConstant.nameMap.put(JSONLibUtil.getString(rack, "uuid"),
                    ...
                    for (int k = 0; k < servers.size(); k++) {
                        ...
                        // 将上级ID放入JSON，用于页面查询
                        server.put("areaid", regionData.getId());
                        ...
                        if (MonitorConstant.shieldmap.get(server.get("uuid")) == null) {
                            ...
                            MonitorConstant.shieldmap.put(JSONLibUtil.getString(server, "uuid"),
            ...
            synchronized (MonitorConstant.roomMap) {
                // 将机房数据存入常量
                MonitorConstant.roomMap.put(regionData.getId() + "", rooms);
            }
            rooms = null;
     ...
     * @param code 故障类型编码
     */
    public void sendTotalEmail(String code) {
        String currenttime = StringUtil.dateToString(new Date(), "yyyy-MM-dd");
        Map<String, Object> condition = new LinkedHashMap<String, Object>();
        condition.put("insert_date", currenttime);
        condition.put("monitor_type", code);
        List<MonitorServerVO> lists = monitorService.getDataByDay(condition);
        if (lists != null && lists.size() > 0) {
            // 拼html
            StringBuffer sb = new StringBuffer();
            sb.append("<table border=\"0\" cellpadding=\"1\"><tr><td style=\"width=200px;font-weight:bold;\">区域</td><td style=\"width=200px;font-weight:bold;\">机房</td><td style=\"width=200px;font-weight:bold;\">机架</td><td style=\"width=200px;font-weight:bold;\">服务器</td></tr>");
                    ...
                    for (int i = 0; i < pools.size(); i++) {
                        ...
                        if (hosts != null && hosts.size() > 0) {
                            ...
                            for (int j = 0; j < hosts.size(); j++) {
                                ...
                                JSONObject host_info = channel.hostQueryInfo(host.get("uuid") + "").getJSONObject(
                                        "host");
                                host.put("poolid", pool.getString("uuid"));
                                host.put("areaid", regionData.getId());
                                host.put("area_name", regionData.getName());
                        ...
                        synchronized (MonitorConstant.hostsMap) {
                            MonitorConstant.hostsMap.put(pool.getString("uuid") + "-" + pool.getString("name") + "-"
                                    + regionData.getName() + "-" + regionData.getId(), hosts);
                        }
                        pool = null;
                        object_host = null;
                        hosts = null;
    ...
    public void startServerMonitor() {
        try {
            ...
            for (RegionData regionData : regionDatas) {
                HttpGatewayAsyncChannel channel_asy = HttpGatewayManager.getAsyncChannel(regionData.getId());
                String[] array = MonitorConstant.getServerObjects(regionData.getId());
    ...
    public void startHostsMonitor() {
        try {
            ...
            for (RegionData regionData : regionDatas) {
                HttpGatewayAsyncChannel channel_asy = HttpGatewayManager.getAsyncChannel(regionData.getId());
                String[] array = MonitorConstant.getHostObjects(regionData.getId());
