...
public class CountUserProductsPriceHelper {
	   ...
	   private static final Logger logger = Logger.getLogger(CloudHostWarehouseHelper.class);
		...
		public BigDecimal getAllOneDayPrice(String userId){
			...
			List<CloudDiskVO> cloudDiskList = cloudDiskMapper.getCloudDiskByUserId(userId);
			for(CloudDiskVO cloudDisk : cloudDiskList){
				totalPrice = totalPrice.add(CloudDiskPrice.getCloudDiskPrice(cloudDisk.getRegion()+"", CapacityUtil.toGBValue(cloudDisk.getDisk(),   0, BigDecimal.ROUND_UP).intValue()+""));
			}
 			//计算VPC的价格(VPC+ip)
 			...
 			for(VpcBaseInfoVO vpcBaseInfo : vpcList){
 				//筛选掉已经停用了的vpc
 				if(vpcBaseInfo.getStatus() == 2 || vpcBaseInfo.getType() != 1){
 					continue;
 				}
 				//加上VPC价格
		...
		/**
		 * 
		 * getAgentAllOneDayPrice: 获取代理商一天费用
		 *
		 * @author sasa
		 * @param userId
		 * @return BigDecimal
		 * @since JDK 1.7
		 */
		public BigDecimal getAgentAllOneDayPrice(String userId){
...
//			CloudDiskMapper cloudDiskMapper                                     = this.sqlSession.getMapper(CloudDiskMapper.class);
			...
			List<CloudHostVO> cloudHostList = cloudHostMapper.getByAgentId(userId);
			if(cloudHostList!=null&&cloudHostList.size()>0){
				for(CloudHostVO vo:cloudHostList){
					//只计算终端用户创建的云主机
					if(vo.getType()==2 || vo.getType()==6){
						if(!(vo.getSummarizedStatusText().equals("创建失败")||vo.getSummarizedStatusText().equals("停机"))&&vo.getMonthlyPrice()!=null){						
							totalPrice = vo.getMonthlyPrice().add(totalPrice);
						}
					}
...
//			}
			//计算VPC的价格(VPC+ip) 
			List<VpcBaseInfoVO> vpcList = vpcBaseInfoMapper.getVpcByAgentId(userId);
			for(VpcBaseInfoVO vpcBaseInfo : vpcList){
				//筛选掉已经停用了的vpc
				if(vpcBaseInfo.getStatus() == 2 || vpcBaseInfo.getType() != 2){
					continue;
				}
				//加上VPC价格
				totalPrice.add(VPCAmountAndPrice.getVpcPrice(vpcBaseInfo.getRegion()+"", vpcBaseInfo.getHostAmount()+""));
			...
			try {
				if(terminalUserVO!=null){
					...
					for(VpcBaseInfoVO vpcBaseInfo : vpcList){
						...
						if(vpcBaseInfo.getHostAmount()>0){							
							// 加上VPC价格
