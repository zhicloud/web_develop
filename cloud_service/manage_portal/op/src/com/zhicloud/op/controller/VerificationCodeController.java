...
public class VerificationCodeController
{
	...
	@RequestMapping("/public/verificationCode/new.do")
	public void publicQueryTermianlUser(HttpServletRequest request, HttpServletResponse response) throws Exception
	{
		logger.debug("VerificationCodeController.publicQueryTermianlUser()");
		...
		// 生成随机加减法的校验码
		this.randomCalculate(g, random, request);
	...
	// 生成随机字母数字的验证码
	private void randomWords(Graphics g,Random random,HttpServletRequest request){
        // 随机产生4位验证码
		String[] codes = {	"2", "3", "4", "5", "6", "7", "8", "9", 
							"A", "B", "C", "D", "E", "F", "G", 
							"H", "J", "K", "L", "M", "N", 
							"P", "Q", "R", "S", "T", 
							"U", "V", "W", "X", "Y", "Z" };
		...
		// 将认证码存入SESSION
		VerificationCodeHelper.setCode(request, code);
	}
	
	// 生成加减法计算的验证码
	private void randomCalculate(Graphics g,Random random,HttpServletRequest request){
		int intTemp; 
        // 随机产生10以内的参与计算的两个参数
        int intFirst = random.nextInt(10);  
        int intSec = random.nextInt(10);  
        //排除相加大于10的情况
        while((intFirst+intSec)>=10){
        	intFirst = random.nextInt(10);  
            intSec = random.nextInt(10); 
        }
        String checkCode = "";  
        int result = 0;
        // 根据随机数选择计算方法
		...
		{
			String str = checkCode.substring(i, i+1); 
			// 将认证码显示到图象中
			g.setColor(new Color(20 + random.nextInt(110), 20 + random.nextInt(110), 20 + random.nextInt(110)));
			// 调用函数出来的颜色相同，可能是因为种子太接近，所以只能直接生成
			g.drawString(str, 5 * i+5, 16);
		}
		// 将认证码存入SESSION 
		VerificationCodeHelper.setCode(request, result+"");
	}
