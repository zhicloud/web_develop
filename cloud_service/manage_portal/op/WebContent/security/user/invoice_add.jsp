    ...
    <jsp:include page="/src/common/tpl/u_header.jsp"></jsp:include>
     ...
     <div class="wrap"><jsp:include page="/src/common/tpl/u_mcslider.jsp"></jsp:include></div>
      ...
      <ul class="steps" style="width:480px;">
        ...
        <li class="l">确认信息<span>3</span></li>
        <li class="l">提交<span>4</span></li>
      </ul>
      <form id="invoice_add_dlg_form" class="wizard" method="post">
        <ul style="width:4000px;height:400px;">
          <li>
            ...
            %>
              <div id="addr1" style="width: 690px;padding: 15px 0;margin:0 auto;line-height:30px;">
                ...
                <input id="addressId" name="addressId"  type="hidden" value = "<%=invoiceAddressVO.getId() %>" />
                  ...
                  <tr>
                    <td width="105">税号</td>
                    <td width="545"><input name="taxNumber" type="hidden" value = "<%=invoiceAddressVO.getTaxNumber() %>" /><%=invoiceAddressVO.getTaxNumber() %></td>
                  </tr>
                  <tr>
                    <td width="105">开户银行、帐号</td>
                    <td width="545"><input name="bankNumber" type="hidden" value = "<%=invoiceAddressVO.getBankNumber() %>" /><%=invoiceAddressVO.getBankNumber() %></td>
                  </tr>
                  <tr>
                    <td width="105">单位地址、电话</td>
                    <td width="545"><input name="addressTel" type="hidden" value = "<%=invoiceAddressVO.getAddressTel() %>" /><%=invoiceAddressVO.getAddressTel() %></td>
                  </tr>
                  <tr>
                    <td>寄送地址</td>
              ...
              %> 
                ...
                <table width="690" border="0" cellspacing="0" cellpadding="5">
                  ...
                  <tr>
                    <td width="105">税号</td>
                    <td width="565"><input id = "taxNumber" name="taxNumber" type="text"  onchange = "insertTaxNumber()" onblur="checkTaxNumber(),submitCheck()" class="textbox" style="height:28px; width:335px"/>
                    <span class="tip" id="tip-taxNumber"></span>
                    </td>
                  </tr>
                  <tr>
                    <td width="105">开户银行、帐号</td>
                    <td width="565"><input id = "bankNumber" name="bankNumber" type="text"  onchange = "insertBankNumber()" onblur="checkBankNumber(),submitCheck()" class="textbox" style="height:28px; width:335px"/>
                    <span class="tip" id="tip-bankNumber"></span>
                    </td>
                  </tr>
                  <tr>
                    <td width="105">单位地址、电话</td>
                    <td width="565"><input id = "addressTel" name="addressTel" type="text"  onchange = "insertAddressTel()" onblur="checkAddressTel(),submitCheck()" class="textbox" style="height:28px; width:335px"/>
                    <span class="tip" id="tip-addressTel"></span>
                    </td>
                  </tr>
...
<!--             <div class="buttonbar" style="border-top:solid 1px #b2b2b2;width: 780px;"><input style="cursor:pointer;" type="button" class="bluebutton r" value="下一步" onclick="nextstep();"/><a class="graybutton r" href="javascript:void(0);" style="cursor:pointer;" onclick="prevstep();">上一步</a><i>合计：<strong><span class = "amount_sum">0</span>元</strong></i><br /> -->
            <div class="buttonbar" style="border-top:solid 1px #b2b2b2;width: 780px;"><input id="add_invoice_dlg_submit_btn" style="cursor:pointer;" type="button" class="bluebutton r" value="下一步" onclick="nextstep(),setValues()" disabled = "disabled"/><a class="graybutton r" href="javascript:void(0);" style="cursor:pointer;" onclick="prevstep();">上一步</a><i>合计：<strong><span class = "amount_sum">0</span>元</strong></i><br />
              请注意：仅当年度充值金额可以索取发票，已索取发票金额不能退款。金额超过<strong>100元</strong>才能索取发票。</div>
          </li>
          <li>
          	<div id="confirm" style="width: 690px;padding: 15px 0;margin:0 auto;line-height:30px;">
                <table width="670" border="0" cellspacing="0" cellpadding="5">
                  <tr>
                    <td width="105">发票抬头</td>
                    <td width="545"><p id="taitou"></p></td>
                  </tr>
                  <tr>
                    <td width="105">税号</td>
                    <td width="545"><p id="shuihao"></p></td>
                  </tr>
                  <tr>
                    <td width="105">开户银行、帐号</td>
                    <td width="545"><p id="zhanghao"></p></td>
                  </tr>
                  <tr>
                    <td width="105">单位地址、电话</td>
                    <td width="545"><p id="danwei"></p></td>
                  </tr>
                  <tr>
                    <td>寄送地址</td>
                    <td><p id="dizhi"></p></td>
                  ...
                  <tr>
                    <td>收件人</td>
                    <td><p id="shoujian"></p></td>
                  </tr>
                  <tr>
                    <td>联系电话</td>
                    <td><p id="dianhua"></p></td>
                  </tr>
                  <tr>
                    <td>总额</td>
                    <td><p class = "amount_sum"></p></td>
                  </tr>
                </table>
              </div>
<!--           	<div class="buttonbar" style="border-top:solid 1px #b2b2b2;width: 780px;"><input style="cursor:pointer;" type="button" class="bluebutton r" value="下一步" onclick="nextstep();"/><a class="graybutton r" href="javascript:void(0);" style="cursor:pointer;" onclick="prevstep();">上一步</a><i>合计：<strong><span class = "amount_sum">0</span>元</strong></i><br /> -->
            <div class="buttonbar" style="border-top:solid 1px #b2b2b2;width: 780px;"><input id="confirm_submit_btn" style="cursor:pointer;" type="button" class="bluebutton r" value="提交" onclick="nextstep();"/><a class="graybutton r" href="javascript:void(0);" style="cursor:pointer;" onclick="prevstep();">上一步</a><i>合计：<strong><span class = "amount_sum">0</span>元</strong></i><br />
              请注意：仅当年度充值金额可以索取发票，已索取发票金额不能退款。金额超过<strong>100元</strong>才能索取发票。</div>
          </li>
          <li>
...
var taxNumberFlag = false;
var bankNumberFlag = false;
var addressTelFlag = false;
	 ...
	 for(i=0;i<div.length;i++){
	  if(myradio[i].checked){
	   ...
	   for (z=0;z<child.length;z++){
		   if (child[z].name=="userId"){
			   data[0] = child[z].value; 
		   }

		   if (child[z].name=="addressId"){
			   data[1] = child[z].value; 
		   }
		   if (child[z].name=="invoiceTitle"){
			   data[2] = child[z].value; 
		   }	
		   
		   if (child[z].name=="address"){
			   data[3] = child[z].value; 
		   }
		   
		   if (child[z].name=="recipients"){
			   data[4] = child[z].value; 
		   }
		   
		   if (child[z].name=="phone"){
			   data[5] = child[z].value; 
		   }
		   if (child[z].name=="taxNumber"){
			   data[6] = child[z].value; 
		   }
		   if (child[z].name=="bankNumber"){
			   data[7] = child[z].value; 
		   }
		   if (child[z].name=="addressTel"){
			   data[8] = child[z].value; 
		   }
	  	 }
	  }
	   else{
...
//----------------
function insertTaxNumber(){
	data[6] = document.getElementById("taxNumber").value;
}

function insertBankNumber(){
	data[7] = document.getElementById("bankNumber").value;
}

function insertAddressTel(){
	data[8] = document.getElementById("addressTel").value;
}

function checkTaxNumber(){
	var taxNumber = data[6];
	if(taxNumber.length > 32){
		$("#tip-taxNumber").html("<font color=red>税号长度不能超过32个字符</font>");
		taxNumberFlag = false;
		return false;
	}
	$("#tip-taxNumber").html("");
...
function checkBankNumber(){
	var bankNumber = data[7];
	if(bankNumber.length > 64){
		$("#tip-bankNumber").html("<font color=red>开户银行及帐号长度不能超过64个字符</font>");
		bankNumberFlag = false;
		return false;
	}
	$("#tip-bankNumber").html("");
	bankNumberFlag = true;
...
function checkAddressTel(){
	var addressTel = data[8];
	if(addressTel.length > 128){
		$("#tip-bankNumber").html("<font color=red>开户银行及帐号长度不能超过128个字符</font>");
		addressTelFlag = false;
		return false;
	}
	$("#tip-addressTel").html("");
	addressTelFlag = true;
...
function setValues(){
	$("#taitou").html(data[2]);
	$("#dizhi").html(data[3]);
	$("#shoujian").html(data[4]);
	$("#dianhua").html(data[5]);
	$("#shuihao").html(data[6]);
	$("#zhanghao").html(data[7]);
	$("#danwei").html(data[8]);
}
...
self.save = function() {
	...
	formData.taxNumber = data[6];
	formData.bankNumber = data[7];
	formData.addressTel = data[8];
