			...
			<table border="0" style="height: auto; margin: auto;">
				...
				<tr>
					...
					<td style="padding:5px 0 5px 0;">
						...
						<div id="region_2">
							<select id="sys_image_id_CD" name="sys_image_id_CD" style="width:225px" class="easyui-radio">
								<option value="">请选择</option>
								<%
									for( SysDiskImageVO sysDiskImageOption : sysDiskImageOptionsCD )
									{
								%>
										<option value="<%=sysDiskImageOption.getId()%>"><%=sysDiskImageOption.getName()%></option>
								<%
				...
				<tr>
					...
					<td >
						<textarea name="description" id="description" rows="6" cols="44" style="resize:none;" onblur="checkDescription()"></textarea>
                    </td>
				...
				<tr>
					...
					<td >
						<input type="text" id="total_amount" name="total_amount" style="position:relative; top:2px;" onblur="checkTotalAmount()"/>
						个
					</td>
					<td id="operator-tip-amount"><i></i></td>
				</tr>
...
function checkDescription(){
	...
	if(description.length>50){
		$("#operator-tip-description").html("<b>输入不能超过50个字符</b>");
		return false;
	}
...
function checkTotalAmount(){	
	var amount = new String($("#total_amount").val());
	if(amount==null || amount==""){
	    $("#operator-tip-amount").html("<b>库存总量不能为空</b>");		
	    return false;
	}
	if(!(/^[0-9]\d*$/.test(amount))){ 
	    $("#operator-tip-amount").html("<b>请输入1-10000的整数</b>");		
		return false;
	}
	if(amount<0||amount>10000){ 
	    $("#operator-tip-amount").html("<b>请输入1-10000的库存总量</b>");		
		return false;
	}
	
    $("#operator-tip-amount").html("<img src='<%=request.getContextPath()%>/images/button_validated_green_16_16.gif' width='16' height='16' alt='正确' />");		
	return true;
} 
...
var _warehouse_add_dlg_scope_ = new function(){
	...
	// 保存
	self.save = function() {
		if(!checkTotalAmount() || !checkDescription()) return;		
