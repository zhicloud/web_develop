...
<body>
<div id="email_template_add_dlg_container">
<form id="email_template_add_dlg_form" method="post">
	...
	<table border="0" cellpadding="0" cellspacing="0">
        <tr>
            ...
            <td style="padding:5px;"><select id="config_id" name="config_id" onblur="checkConfig()" style="width:150px;height:26px;font-family:'微软雅黑';vertical-align:middle;">
                ...
                <%
                    for( EmailConfigVO emailConfigVO : emailConfigList)
                    {
                %>
                    <option value="<%=emailConfigVO.getId()%>"><%=emailConfigVO.getName()%></option>
                <%
                    }
                %>
            </select>
		...
		<tr>
            <td style="vertical-align:middle; text-align:right;">
                ...
                邮件主题：</td>
            <td style="padding:5px;">
                <input type="text" id="subject" name="subject" style="width:300px;" onblur="checkSubject()"/>
            </td>
		...
		<tr>
            <td style="vertical-align:middle; text-align:right;">
                ...
                邮件正文：</td>
            <td style="padding:5px;">
			    <textarea id="content" name="content" style="width:300px;height:200px; visibility:hidden;" onclick="checkContent()"></textarea>
            </td>
            <td><span id="template-tip-content"></span></td>
            <%--<td>--%>
                <%--<select name=mytextarea size=2>--%>
                    <%--<option onclick="insertText(document.getElementById('content'),'呵呵')"> 用户名 </option>--%>
                    <%--<option> two </option>--%>
                    <%--<option> three </option>--%>
                    <%--<option> four </option>--%>
                <%--</select>--%>
        ...
        <tr>
            ...
            <td align="right" style="padding:5px;">
                <a href="javascript:" class="easyui-linkbutton" id="email_template_add_dlg_save_btn">
                    &nbsp;保&nbsp;存&nbsp;
                </a>
                &nbsp;&nbsp;
                <a href="javascript:" class="easyui-linkbutton" id="email_template_add_dlg_close_btn">
                    &nbsp;关&nbsp;闭&nbsp;
                </a>
            </td>
        </tr>
...
<script type="text/javascript">
    ...
    function checkRecipient(){
        ...
        var reg = new RegExp("^((([a-zA-Z0-9_\.-]+)@([\da-zA-Z\.-]+)\.([a-zA-Z\.]{2,6}\;))*(([a-zA-Z0-9_\.-]+)@([\da-zA-Z\.-]+)\.([a-zA-Z\.]{2,6})))$"); 
    ...
    var _email_template_add_dlg_scope_ = {};		// 作用域
    
    // 保存
    _email_template_add_dlg_scope_.save = function()
    {
        var formData = $.formToBean(email_template_add_dlg_form);
        var content = new String($("#content").val());
        content.replace(new RegExp('(["\"])', 'g'),"\\\"");
        formData.content = content.replace(new RegExp('(["\n\r"])', 'g'),'');
        ajax.remoteCall("bean://emailTemplateService:addTemplate",
                [formData],
                function(reply)
                {
    ...
    // 关闭
    _email_template_add_dlg_scope_.close = function()
    {
        window.location.href = "<%=request.getContextPath()%>/bean/page.do?userType=<%=userType%>&bean=emailTemplateService&method=managePage";
    };
    
    // 保存
    $("#email_template_add_dlg_save_btn").click(function(){
        if(checkConfig() && checkName() && checkSubject() && checkContent()){
            _email_template_add_dlg_scope_.save();
        }
    });

    // 关闭
    $("#email_template_add_dlg_close_btn").click(function(){
        _email_template_add_dlg_scope_.close();
    });
