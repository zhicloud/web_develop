...
<div id="mail_config_mod_dlg_container">
	<div id="mail_config_mod_dlg" class="easyui-dialog" title="修改配置"
			style="
				width:470px;
				height:250px;
				padding:10px;
			...
			data-options="
				iconCls: 'icon-edit',
				buttons: '#mail_config_mod_dlg_buttons',
				modal: true,
				onMove:_mail_config_mod_dlg_scope_.onMove,
				onClose: function(){
					$(this).dialog('destroy');
				},
				onDestroy: function(){
					delete _mail_config_mod_dlg_scope_;
				}
			">
		<form id="mail_config_mod_dlg_form" method="post">
			<input type="hidden" name="id" value="<%=mailConfigVO.getId()%>"/>
		<table border="0" cellpadding="0" cellspacing="0">
		    <tr>
		        ...
		        <td class="inputcont"><input class="textbox inputtext" type="text" id="name" name="name" onblur="checkName()" value="<%=mailConfigVO.getName()%>" oldname="<%=mailConfigVO.getName()%>"/></td>
		        <td class="inputtip" id="config-tip-name"><i></i></td>
			</tr>
			<tr>
				<td class="inputtitle"><img src="<%=request.getContextPath() %>/images/button_required_red_16_16.gif" width="12" height="12" alt="必填" />发件人：</td>
				<td class="inputcont"><input class="textbox inputtext" type="text" id="sender" name="sender" onblur="checkSender()" value="<%=mailConfigVO.getSender()%>"/></td>
				<td class="inputtip" id="config-tip-sender"><i></i></td>
		    </tr>
			<tr>
				...
				<td class="inputcont"><input class="textbox inputtext" type="text" id="address" name="address" onblur="checkAddress()" value="<%=mailConfigVO.getSenderAddress()%>"/></td>
				<td class="inputtip" id="config-tip-address"><i></i></td>
			</tr>
			<tr>
				...
				<td class="inputcont"><input class="textbox inputtext" type="password" id="password" name="password" onblur="checkPassword()" value="<%=mailConfigVO.getPassword()%>"/></td>
				<td class="inputtip" id="config-tip-password"><i></i></td>
			</tr>
		</table>
		</form>
	</div>
	
	<div id="mail_config_mod_dlg_buttons">
		<a href="javascript:" class="easyui-linkbutton" id="mail_config_mod_dlg_save_btn">
			&nbsp;保&nbsp;存&nbsp;
		</a>
		<a href="javascript:" class="easyui-linkbutton" id="mail_config_mod_dlg_close_btn">
			&nbsp;关&nbsp;闭&nbsp;
		</a>
	</div>
</div>
...
function checkName(){
	...
	var old = new String($("#name").attr("oldname")).trim();
	var flag = true;
	if(name == null || name == ""){
...
//================================
var _mail_config_mod_dlg_scope_ = {};		// 作用域
_mail_config_mod_dlg_scope_.onMove = function(){
	var thisId = "#mail_config_mod_dlg";
...
// 保存
_mail_config_mod_dlg_scope_.save = function()
{
	var formData = $.formToBean(mail_config_mod_dlg_form);
	ajax.remoteCall("bean://emailConfigService:updateConfigById",
		[formData],
		function(reply)
		{
			...
			{
				var data = $("#mail_config_mod_dlg_container").parent().prop("_data_");
				$("#mail_config_mod_dlg").dialog("close");
...
// 关闭
_mail_config_mod_dlg_scope_.close = function()
{
	$("#mail_config_mod_dlg").dialog("close");
};
...
// 保存
$("#mail_config_mod_dlg_save_btn").click(function(){
	if(checkName() && checkSender() && checkAddress() && checkPassword()){
		_mail_config_mod_dlg_scope_.save();
	}
});

// 关闭
$("#mail_config_mod_dlg_close_btn").click(function(){
	_mail_config_mod_dlg_scope_.close();
});
