...
<%@ page import="com.zhicloud.op.vo.SmsTemplateVO" %>
<%@ page import="java.util.List" %>
<%@page pageEncoding="utf-8" contentType="text/html; charset=utf-8"%>
<%
    Integer userType = AppConstant.SYS_USER_TYPE_OPERATOR;
    LoginInfo loginInfo = LoginHelper.getLoginInfo(request, userType);
    SmsTemplateVO smsTemplateVO = (SmsTemplateVO)request.getAttribute("smsTemplateVO");
...
<body>
<div id="sms_template_mod_dlg_container">
    <form id="sms_template_mod_dlg_form" method="post">
        <input type="hidden" id="id" name="id" value="<%=smsTemplateVO.getId()%>"/>
        ...
        <table border="0" cellpadding="0" cellspacing="0">
            <tr>
                ...
                <td style="padding:5px;"><select id="config_id" name="config_id" onblur="checkConfig()" style="width:150px;height:26px;font-family:'微软雅黑';vertical-align:middle;">
                    ...
                    <%
                        ...
                        {
                            if (smsConfigVO.getId().equals(smsTemplateVO.getConfigId())){
                    %>
                    <option value="<%=smsConfigVO.getId()%>" selected="selected"><%=smsConfigVO.getConfigName()%></option>
                    <%
                            continue;
                        }
                    %>
                    <option value="<%=smsConfigVO.getId()%>"><%=smsConfigVO.getName()%></option>
                    <%
                        }
                    %>
                </select>
            ...
            <tr>
                ...
                <td style="padding:5px;">
                    <input type="text" id="name" name="name" value="<%=smsTemplateVO.getName()%>" old="<%=smsTemplateVO.getName()%>" style="width:300px;" onblur="checkName()"/>
                </td>
            ...
            <tr>
                <td style="vertical-align:middle; text-align:right;">
                    ...
                    标识码：</td>
                <td style="padding:5px;">
                    <input type="text" id="code" name="code" value="<%=smsTemplateVO.getCode()%>" style="width:300px;" onblur="checkCode()"/>
            ...
            <tr>
                <td style="vertical-align:middle; text-align:right;">
                    收件人：</td>
                <td style="padding:5px;">
                    <input type="text" id="recipient" name="recipient" value="<%=smsTemplateVO.getRecipient()%>" style="width:300px;" onblur="checkRecipient()"/>
                    (多个收件人用分号隔开)
                </td>
            ...
            <tr>
                <td style="vertical-align:middle; text-align:right;">
                    ...
                    短信正文：</td>
                <td style="padding:5px;">
                    <textarea id="content" name="content" style="width:650px;height:200px;" onclick="checkContent()"><%=smsTemplateVO.getContent()%></textarea>
                </td>
            ...
            <tr>
                ...
                <td align="right" style="padding:5px;">
                    <a href="javascript:" class="easyui-linkbutton" id="sms_template_mod_dlg_save_btn">
                        &nbsp;保&nbsp;存&nbsp;
                    </a>
                    &nbsp;&nbsp;
                    <a href="javascript:" class="easyui-linkbutton" id="sms_template_mod_dlg_close_btn">
                        &nbsp;关&nbsp;闭&nbsp;
                    </a>
                </td>
            </tr>
...
<script type="text/javascript">
    ...
    function checkContent(){
        ...
        var _sms_template_mod_dlg_scope_ = {};		// 作用域

        // 保存
        _sms_template_mod_dlg_scope_.save = function () {
            var formData = $.formToBean(sms_template_mod_dlg_form);
            var content = new String($("#content").val());
            content.replace(new RegExp('(["\"])', 'g'),"\\\"");
            formData.content = content.replace(new RegExp('(["\n\r"])', 'g'),'');
            ajax.remoteCall("bean://smsTemplateService:updateTemplateById",
                    [formData],
                    function (reply) {
        ...
        // 关闭
        _sms_template_mod_dlg_scope_.close = function () {
            window.location.href = "<%=request.getContextPath()%>/bean/page.do?userType=<%=userType%>&bean=smsTemplateService&method=managePage";
        };

        // 保存
        $("#sms_template_mod_dlg_save_btn").click(function () {
            if (checkConfig() && checkName() && checkCode() && checkRecipient() && checkContent()) {
                _sms_template_mod_dlg_scope_.save();
            }
        });

        // 关闭
        $("#sms_template_mod_dlg_close_btn").click(function () {

            _sms_template_mod_dlg_scope_.close();
        });
