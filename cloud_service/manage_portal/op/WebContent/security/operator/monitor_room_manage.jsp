	...
	<div class="tab-container">
		<div id="toolbar">
			<div style="display: table; width: 100%;">
				<div class="f-mb5" style="line-height:24px;">
					...
					<% if(!"room".equals(menuflag)) {%><a class="easyui-linkbutton oper-btn-sty" href="javascript:;" data-options="plain:true,iconCls:'icon-redo'" style="float:right;" id="back_area_btn">返回区域</a><%}else{ }%>
				</div>
		...
		<table id="area_datagrid" class="easyui-datagrid" title="<%=areaname %> >> 机房信息" data-options="url: '<%=request.getContextPath()%>/bean/ajax.do?userType=<%=userType%>&bean=monitorService&method=roomQuery&areaid=<%=areaid %>&menuflag=<%=menuflag %>',queryParams: {},toolbar: '#toolbar',rownumbers: false,striped: true,remoteSort:false,fitColumns: true,pagination: true,pageSize: 20,view: createView(),onLoadSuccess: onLoadSuccess">			
		<% }else{ %>
		<table id="area_datagrid" class="easyui-datagrid" title="机房信息" data-options="url: '<%=request.getContextPath()%>/bean/ajax.do?userType=<%=userType%>&bean=monitorService&method=roomQuery&areaid=<%=areaid %>&menuflag=<%=menuflag %>',queryParams: {},toolbar: '#toolbar',rownumbers: false,striped: true,remoteSort:false,fitColumns: true,pagination: true,pageSize: 20,view: createView(),onLoadSuccess: onLoadSuccess">
		<% } %>
			<thead>
				<tr>
					...
					<% } %>
					<th data-options="field:'name',width:100">机房名</th>
