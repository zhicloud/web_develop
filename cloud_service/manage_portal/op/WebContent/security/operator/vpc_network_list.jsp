...
<div id="exported_dlg_container">
	<div id="exported_dlg" class="easyui-dialog" title="端口绑定列表-[<%=vpcBaseInfoVO.getDisplayName() %>]"
		...
		<form id="big_form"  method="post">
			<table id="exported_datagrid" class="easyui-datagrid"
				style=""
				data-options="
					url: '<%=request.getContextPath()%>/bean/ajax.do?userType=<%=userType%>&bean=vpcService&method=getAllBindPort&id=<%=vpcId %>',
					queryParams: {},
					rownumbers: true,
					striped: true,
					remoteSort:false,
				...
				<thead>
					<tr>
						<th data-options="field:'displayName',width:50" >主机</th>
						<th data-options="field:'outerIp',width:50">外网IP</th>
						<th data-options="field:'outerPort',width:30">外网端口</th>
						<th data-options="field:'hostPort',width:30">主机端口</th>
						<th data-options="field:'protocol',width:30" formatter="formatProtocol">协议</th>
						<th data-options="field:'createTime',width:70" formatter="timeFormat" sortable=true>绑定时间</th>
					</tr>
				</thead>
			</table>
		</form>
