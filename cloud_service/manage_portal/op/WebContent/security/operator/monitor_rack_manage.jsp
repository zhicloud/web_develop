	...
	<body style="visibility:hidden;">
		...
		<% if(!"rack".equals(menuflag)) {%>
			<table id="area_datagrid" class="easyui-datagrid" title="<%=areaname %> >> <%=roomname %> >> 机架信息"
				style=""
				data-options="
					url: '<%=request.getContextPath()%>/bean/ajax.do?userType=<%=userType%>&bean=monitorService&method=rackQuery&roomid=<%=roomid %>&menuflag=<%=menuflag %>',
					queryParams: {},
					toolbar: '#toolbar',
					rownumbers: true,
					striped: true,
				...
				">
				<%}else{ %>
			<table id="area_datagrid" class="easyui-datagrid" title="机架信息"
				style=""
				data-options="
					url: '<%=request.getContextPath()%>/bean/ajax.do?userType=<%=userType%>&bean=monitorService&method=rackQuery&roomid=<%=roomid %>&menuflag=<%=menuflag %>',
					queryParams: {},
				...
				<thead>
					<tr>
						...
						<%} %>
						<th field="name" width="100px">机架名</th>
						<th field="status" formatter="statusColumnFormatter"  width="60px" align="center">当前状态</th>
						<th field="operate" formatter="agentColumnFormatter" width="100px">操作</th>
					</tr>
				</thead>
			</table>
			...
			<span class="round_stop">●</span> <span style="font-size: 14px;" id="shieldspan"></span>
						<% if(!"rack".equals(menuflag)) {%>
							<a href="#" style="float:right;" class="easyui-linkbutton" iconCls="icon-redo" id="back_area_btn">返回机房</a>
						<%}else{ }%>
					</div>
					<% if(!"rack".equals(menuflag)) {}else{ %>
