...
<%
	...
	SysGroupVO sysGroupVO = (SysGroupVO)request.getAttribute("sysGroup");	
%>
<div id="sys_group_set_member_dlg_container">

	<div id="sys_group_set_member_dlg" class="easyui-dialog" title="权限组组员设置-[<%=sysGroupVO.getGroupName()%>]" 
			style="
				width:1000px; 
				height:500px; 
				padding:10px;
				...
				<tr>
					<td>
						...
						<table id="left_datagrid" class="easyui-datagrid" title="可选"  
							...
							"
							data-options="
								url: '<%=request.getContextPath()%>/bean/ajax.do?userType=<%=userType%>&type=2&bean=sysUserService&method=querySysUserNotInGroupId',
								queryParams: {
									groupId: '<%=groupId%>'
								},
...
var _sys_group_set_member_dlg_scope_ = new function(){
	...
	$(document).ready(function(){
		...
		$("#left_query_btn").click(function(){
			...
			queryParams.type    = 2; //$("#left_user_type").val().trim();
