...
<!-- 		    </tr> -->
		     <tr>
		      ...
		      <td class="inputcont"><input class="textbox inputtext" type="text" id="name" name="name" onblur="checkName()" value="<%=operator.getName()==null?"":operator.getName()%>"/></td>
		      <td class="inputtip" id="admin-tip-name"><i>请输入2-16个字符的昵称</i></td>
		    </tr> 
		    <tr>
...
function checkName(){
	...
	if(myName==null || myName==""){
		$("#admin-tip-name").html("<b>昵称不能为空</b>");
		return false;
	}
	if(myName.length<2 || myName.length>16){
		$("#admin-tip-name").html("<b>昵称长度为2-16个字符</b>");
		return false;
	}
...
var _operator_mod_dlg_scope_ = new function(){
	...
	this.save = function() {
		...
		ajax.remoteCall("bean://operatorService:updateOperatorById",
			...
			function(reply) {
				...
				{
					if(reply.result.message.indexOf("昵称")>=0){
						$("#admin-tip-name").html("<b>"+reply.result.message+"</b>");
					}else{
	...
	$(document).ready(function(){
		...
		$("#operator_mod_dlg_save_btn").click(function(){
			if(checkGroupId()& checkName() &checkEmail()& checkPhone()){
				self.save();
			}
		});
		// 关闭
