...
function onLoadSuccess()
{
	...
	//启动
	$("#start_host_btn").click(function(){
		var d = $("#tb").datagrid("getSelected");
		if(d==null){
			top.$.messager.alert("提示","请选择您要启动的云主机","info",function(){
			});
			return;
		}
		if(d.status==1 && d.runningStatus==1){
		}else {
			top.$.messager.alert("提示","只有关机的云主机才能启动","info",function(){
			});
			return;
		}
		var id = d.id;
		top.$.messager.confirm("确认", "确定要启动该云主机吗？", function (r) { 
			if(r){
	...
	//关机
	$("#shutdown_host_btn").click(function(){
		var d = $("#tb").datagrid("getSelected");
		if(d==null){
			top.$.messager.alert("提示","请选择您要关闭的云主机","info",function(){
			});
			return;
		}
		if(d.status==1 && d.runningStatus==2){
		}else {
			top.$.messager.alert("提示","只有已启动的云主机才能关闭","info",function(){
			});
			return;
		}
		var id = d.id;
		top.$.messager.confirm("确认", "确定要关闭该云主机吗？", function (r) { 
			if(r){
	...
	//创建云硬盘
	$("#create_disk").click(function(){
		var id = "<%=terminalUser.getId()%>";
		window.location.href="<%=request.getContextPath()%>/bean/page.do?userType=<%=userType%>&bean=cloudDiskService&method=addPageForAgent&terminalUserId="+encodeURIComponent(id);
	});
	//查看用户操作
...
<%--           <div style="display:block;margin:0 auto; width:238px; height:90px; background:url(<%=request.getContextPath()%>/image/sidebar_storage_bg.png) no-repeat center center; padding:49px 0 15px 0; font-size:36px; line-height:40px; text-align:center; color:#626262;">0</div> <a class="greenbutton" href="#" style="margin:0 auto; width:80px; height:24px; line-height:24px; font-size:12px;">创建云硬盘</a> --%>
      ...
      <div class="r" style="width:650px;">
        <div class="titlebar" style="width:650px;">
          <div class="l links"><a href="#" >云主机</a> &nbsp;&nbsp; |&nbsp;&nbsp; <a href="#"  class="active">云硬盘</a> &nbsp;&nbsp;|&nbsp;&nbsp; <a href="<%=request.getContextPath()%>/bean/page.do?userType=<%=userType%>&bean=terminalUserService&method=agentViewVpcPage&terminalUserId=<%=terminalUser.getId()%>"  >VPC</a>
<!--           	｜　<a href="#">云硬盘</a> -->
          </div>
        </div>
        <div class="titlebar" style="width:650px;padding:15px 0 5px 0;">
      <div class="blocks l">
         <div class="l"><a id="view_detail_btn" href="#" class="bluebutton" style="width:76px">删除</a></div> 
         <div class="l"><a id="view_detail_btn" href="#" class="bluebutton" style="width:76px">恢复</a></div> 
         <div class="clear"></div>
      </div>
        ...
        <table id="tb" class="easyui-datagrid"
          ...
          <thead>
            ...
            <th data-options="field:'type',formatter:formatType,width:43">地域</th>
               <th data-options="field:'name',width:153">名称</th>  
              <th data-options="field:'disk',formatter:formatCapacity,width:67">硬盘容量</th>
              </tr>
          </thead> 
        </table>
