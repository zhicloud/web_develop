...
ajax.async = false;
var priceType = "3"; 
if('<%=cloudHost.getType()%>'=='5' || '<%=cloudHost.getType()%>'=='6'){
	priceType = "2";
}
...
//		save();
	...
	var curNewPrice = refreshPrice(region,priceType,core,memory,dataDisk,bandWidth);
...
function updatePrice(){
	...
	var dataDisk = "<%=CapacityUtil.toGBValue(cloudHost.getDataDisk(), 0)%>";
<%-- 	var bandWidth = "<%=FlowUtil.toMbpsValue(cloudHost.getBandwidth(),  0)%>"; --%>
	var curNewPrice = refreshPrice(region,priceType,core,memory,dataDisk,curBandwidth+"");
	var newPrice = (curNewPrice / 1).toFixed(2);
	var newPrice_hour = (newPrice / 30/ 24).toFixed(2);
	$("#current_core").html(core);
	$("#current_memory").html(memory);
	$("#current_bandwidth").html(curBandwidth+"MB/s");
	$("#price_hour").html(newPrice_hour);
	$("#price").html(newPrice);
	return;
}
